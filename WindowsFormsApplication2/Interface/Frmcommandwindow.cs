...
" ORDER BY TblIssuedetails.IssueCode ");
        ...
        private void button4_Click(object sender, EventArgs e)
        {
            ...
            if (sql.Length > 6)
            {
                if (sql.Substring(0, 6).ToLower() == "select")
                {
                    CommonClass.Ds = CommonClass._Dbtask.ExecuteQueryAzureServer(sql);
                    Gridmain.DataSource = CommonClass.Ds.Tables[0];
                }
                else
                {
        ...
        private void button5_Click(object sender, EventArgs e)
        {
            try
            {
                _dbtask.ExecuteQueryAzureServer("delete from tblbusinessissue");
                _dbtask.ExecuteQueryAzureServer("delete from tblissuedetails");
                _dbtask.ExecuteQueryAzureServer("delete from tblgeneralledger");
                _dbtask.ExecuteQueryAzureServer("delete from tblinventory");
                _dbtask.ExecuteQueryAzureServer("delete from tblitemmaster");
                _dbtask.ExecuteQueryAzureServer("delete from tblparamlist");
                _dbtask.ExecuteQueryAzureServer("delete from tblbatch");
                _dbtask.ExecuteQueryAzureServer("delete from tblitemcategory");
                _dbtask.ExecuteQueryAzureServer("delete from tblaccountgroup");
                _dbtask.ExecuteQueryAzureServer("delete from tblaccountledger");
                _dbtask.ExecuteQueryAzureServer("delete from tbltransactionreceipt");
                _dbtask.ExecuteQueryAzureServer("delete from tblreceiptdetails");
