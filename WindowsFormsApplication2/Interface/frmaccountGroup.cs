...
namespace WindowsFormsApplication2
{
    public partial class frmaccountGroup : Form
    {
        ...
        private void frmaccountGroup_Load(object sender, EventArgs e)
        {
           ...
           CommonClass._Nextg.ClearControles(this);
            ...
            else
            {
                ...
                string ds = _Dbtask.ExecuteScalarAzureServer("select * from tblaccountgroup where agroupid='" + Id + "'");
                if (ds != "")
                {
                    Ds = _Dbtask.ExecuteQueryAzureServer("select * from tblaccountgroup where agroupid='" + Id + "'");
                }
                else
                {
        ...
        public void DeleteAccountgroupAzureServer()
        {
            _Dbtask.ExecuteNonQueryAzureServer("Delete from tblaccountgroup where agroupid='" + TxtId.Text + "'");
        }
    }
}
