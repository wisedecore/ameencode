...
namespace WindowsFormsApplication2
{
    public class DBTask
    {
        ...
        public string _ConnectionStringServer;
        ...
        public void setconstr()
        {
            ...
            _ConnectionStringServer = "Server=tcp:zshopweb.database.windows.net,1433;Initial Catalog=zshopweb;Persist Security Info=False;User ID=wisedecore;Password=wise@123;MultipleActiveResultSets=False;Encrypt=True;TrustServerCertificate=False;Connection Timeout=120;";
        ...
        public DataSet ExecuteQueryAzureServer(string strQuery)
        {
            if (SQLCON.SQlConServer != null)
            {
                if (SQLCON.SQlConServer.State.ToString() == "Open")
                {
                    SQLCON.SQlConServer.Close();
                }

            }


            SQLCON.SQlConAzureServeropen();
            ...
            cmdMaxid.CommandText = strQuery;
            cmdMaxid.CommandTimeout = 0;
            cmdMaxid.Connection = SQLCON.SQlConServer;
            //SqlDataAdapter SQLDA = new SqlDataAdapter(strQuery, SQLCON.SQlCon, cmdMaxid);
            SqlDataAdapter SQLDA = new SqlDataAdapter(cmdMaxid);
        ...
        public bool IsInternetAvailable()
        {
            try
            {
                ...
                {
                    PingReply reply = ping.Send("8.8.8.8", 3000); // Google's DNS
        ...
        public bool ExecuteNonQueryAzureServer_SP(string ProcedureName, object[,] ParamArrayName)
        {
            SQLCON.SQlConAzureServeropen();

            SqlCommand SQLCmd = new SqlCommand();
            SQLCmd.Connection = SQLCON.SQlConServer;
            SQLCmd.CommandText = ProcedureName;
            SQLCmd.CommandType = CommandType.StoredProcedure;

            SqlTransaction SqlTran;
            SqlTran = SQLCON.SQlConServer.BeginTransaction();
        ...
        public string ExecuteScalarAzureServer(string qry)
        {
            if (SQLCON.SQlConServer != null)
            {
                if (SQLCON.SQlConServer.State.ToString() == "Open")
        ...
        public bool ExecuteNonQueryAzureServer(string qry)
        {
            if (SQLCON.SQlConServer != null)
            {
                if (SQLCON.SQlConServer.State.ToString() == "Open")
