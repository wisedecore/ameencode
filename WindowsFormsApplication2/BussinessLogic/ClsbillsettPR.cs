...
namespace WindowsFormsApplication2
{
    class ClsbillsettPR
    {
            ...
            object[,] objArg = new object[7, 2]
        {
            ...
            _Dbtask.ExecuteNonQuery_SP("InsertPRbillsett", objArg);
            return;
        }


        public string GETfeildBYPR(string agvno, string ledcode, string feild)
        {
            string ansr = _Dbtask.znllString(_Dbtask.ExecuteScalar("select  " + feild + "  from tblPRbillsett where ledcode='" + ledcode + "' and agvno='" + agvno + "' and vtype='R' "));
      

           return ansr;
        }

        public string GETfeildBYPRAMT(string agvno, string ledcode, string feild)
        {
            string ansr = _Dbtask.znllString(_Dbtask.ExecuteScalar("select  SUM(" + feild + ")  from tblPRbillsett where ledcode='" + ledcode + "' and agvno='" + agvno + "' and vtype='R' "));


            return ansr;
        }

        public void InsertBillsettlementPR(string Pvno, string Pvtype, long PLedcode, double PDbamt, double PCramt, string PAgvno, DateTime Pdt, int PEmp)
        {
            if (PLedcode != 1)
            {
                Vno = Pvno;
        ...
        {
            _Dbtask.fillDatainCombowithQuery1(Comb, "vno", "tblPRbillsett", "select * from tblPRbillsett where ledcode ='" + FPartyid + "' and vtype='PR' ");
        }
        ...
        public double IsshowBill(string PLedcode, string PAgvno, DateTime PdateFrom, DateTime Pdateto, string Pemp)
        {
            ...
            if (PAgvno == "" || PAgvno == null)
            {
                if (Pemp == "" || Pemp == null)
                {
                    Amt = _Dbtask.znullDouble(_Dbtask.ExecuteScalar("select sum(cramt-dbamt) from tblPRbillsett where ledcode='" + PLedcode + "' and dt between '" + PdateFrom.ToString("MM-dd-yyyy") + " 00:00:00' and '" + Pdateto.ToString("MM-dd-yyyy") + " 23:59:59' "));
                }
                else
                {
                    Amt = _Dbtask.znullDouble(_Dbtask.ExecuteScalar("select sum(cramt-dbamt) from tblPRbillsett where ledcode='" + PLedcode + "' and dt between '" + PdateFrom.ToString("MM-dd-yyyy") + " 00:00:00' and '" + Pdateto.ToString("MM-dd-yyyy") + " 23:59:59' and emp in(" + Pemp + ") "));
                }
            }
            else
            {
                if (Pemp == "" || Pemp == null)
                {
                    Amt = _Dbtask.znullDouble(_Dbtask.ExecuteScalar("select isnull(sum(dbamt),0) from tblPRbillsett where ledcode='" + PLedcode + "' and agvno='" + PAgvno + "'  and dt between '" + PdateFrom.ToString("MM-dd-yyyy") + " 00:00:00' and '" + Pdateto.ToString("MM-dd-yyyy") + " 23:59:59' "));
                }
                else
                {
                    Amt = _Dbtask.znullDouble(_Dbtask.ExecuteScalar("select isnull(sum(dbamt),0) from tblPRbillsett where ledcode='" + PLedcode + "' and agvno='" + PAgvno + "'  and dt between '" + PdateFrom.ToString("MM-dd-yyyy") + " 00:00:00' and '" + Pdateto.ToString("MM-dd-yyyy") + " 23:59:59' and emp in(" + Pemp + ") "));
                }
            }
        ...
        public double correspondngPR(string AGVNO, string ledcode1)
        {
            double SR = _Dbtask.znlldoubletoobject(_Dbtask.ExecuteScalar("select sum(cramt) from tblPRbillsett where vtype='PR' and agvno='" + AGVNO + "' and ledcode='" + ledcode1 + "'"));
        ...
        {
            double SR = _Dbtask.znlldoubletoobject(_Dbtask.ExecuteScalar("select sum(dbamt) from tblPRbillsett where VTYPE='R' and agvno='" + AGVNO + "' and ledcode='" + ledcode1 + "'"));
            double sramt = _Dbtask.znlldoubletoobject(SR);
        ...
        {
            double CHQ = _Dbtask.znlldoubletoobject(_Dbtask.ExecuteScalar("select sum(dbamt) from tblPRbillsett where vtype='ChR'  and agvno='" + AGVNO + "' and ledcode='" + ledcode1 + "'"));
        ...
        public DataSet ShowBillsett(string PLedcode, string PVtype, DateTime Pdatefrom, DateTime Pdateto, string Pemp)
        {
            if (Vtype == "")
            {
                if (Pemp == "" || Pemp == null)
                {
                    return _Dbtask.ExecuteQuery("select * from tblPRbillsett where ledcode='" + PLedcode + "' and dt between '" + Pdatefrom.ToString("MM-dd-yyyy") + " 00:00:00' and '" + Pdateto.ToString("MM-dd-yyyy") + " 23:59:59'  order by dt asc");
                }
                else
                {
                    return _Dbtask.ExecuteQuery("select * from tblPRbillsett where ledcode='" + PLedcode + "' and dt between '" + Pdatefrom.ToString("MM-dd-yyyy") + " 00:00:00' and '" + Pdateto.ToString("MM-dd-yyyy") + " 23:59:59' and emp in(" + Pemp + ") order by dt asc");
                }
            }

            if (Pemp == "" || Pemp == null)
            {
                return _Dbtask.ExecuteQuery("select * from tblPRbillsett where ledcode='" + PLedcode + "' and vtype='" + PVtype + "' and dt between '" + Pdatefrom.ToString("MM-dd-yyyy") + " 00:00:00' and '" + Pdateto.ToString("MM-dd-yyyy") + " 23:59:59' order by dt asc");
            }
            else
            {
                return _Dbtask.ExecuteQuery("select * from tblPRbillsett where ledcode='" + PLedcode + "' and vtype='" + PVtype + "' and dt between '" + Pdatefrom.ToString("MM-dd-yyyy") + " 00:00:00' and '" + Pdateto.ToString("MM-dd-yyyy") + " 23:59:59' and emp in (" + Pemp + ") order by dt asc");
            }
        }
        ...
        public void showcustmrbillss(DataGridView Grid, string PLedcode)
        {
            ...
            DataSet ds2 = _Dbtask.ExecuteQuery("select * from tblPRbillsett where ledcode='" + PLedcode + "' and vtype='PR' ");
            for (int k = 0; k < ds2.Tables[0].Rows.Count; k++)
            {
                Bvno = ds2.Tables[0].Rows[k]["vno"].ToString();
                Bamt = _Dbtask.znlldoubletoobject(ds2.Tables[0].Rows[k]["cramt"]);
                SRAMTT = correspondngPR(Bvno, PLedcode);
        ...
        {
            string Qry = "Delete From tblPRbillsett Where Vno = " + Vno + " And Vtype = '" + Vtype + "'";
        ...
        {
            string Sql = "Select " + Field + " from tblPRbillsett Where vno = '" + Vno + "' And Vtype = '" + Vtype + "'";
        ...
        public void ShowbillBaseonledger(DataGridView Grid, string PLedcode, DateTime Pdate)
        {
            ...
            Grid.Columns.Add("clnsalesamt", "Purchase return Amt");
            ...
            //{
            Ds1 = CommonClass._PRbill.ShowBillsett(PLedcode, "PR", frmod, DateTime.Now, "");
            for (int k = 0; k < Ds1.Tables[0].Rows.Count; k++)
            {
                ...
                BRecamt = CommonClass._PRbill.IsshowBill(PLedcode, Bvno, frmod, DateTime.Now, "");
                if (Bamt > BRecamt)
                {
