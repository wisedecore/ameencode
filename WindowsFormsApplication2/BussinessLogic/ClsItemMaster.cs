   ...
   public static  string depotnobatch="";
     ...
     string BunitF, int UsingmechineF, string ItemnoteStrF)
        ...
        public void InsertItemsAzure()
        {
            ...
            _Dbtask.ExecuteNonQueryAzureServer_SP("InsertItemMaster", objArg);
            return;
        }
...
//               " order by TblItemMaster.itemname asc ");
        ...
        public DataSet ShowItemsBaseonCategoryAzure(string Where)
        {
            Ds = _Dbtask.ExecuteQueryAzureServer("SELECT     TblItemMaster.ItemId, TblItemMaster.Itemcode, TblItemMaster.ItemName, TblItemMaster.MRP, TblItemMaster.SRate, TblItemMaster.PRate, " +
                          "TblItemCategory.Category FROM         TblItemMaster INNER JOIN " +
                          "TblItemCategory ON TblItemMaster.CategoryId = TblItemCategory.CategoryId   INNER JOIN  tblinventory ON  TblItemMaster.ITEMID = tblinventory.PCODE    " + depotnobatch + "  " +
                          " and status=1  " + Where + "  ");
        ...
        public string SpecificFieldAzure(string Itemid, string FiledName)
        {
            string Specific;
            Specific = _Dbtask.ExecuteScalarAzureServer("select " + FiledName + " from TblItemmaster where itemid='" + Itemid + "' and Status=1");
            return Specific;
        }
