    ...
    class ClsTransactionRceipt
    {
       ...
       /* For opening Stock Id*/
            ...
            object[,] objArg = new object[32, 2]
        {
            ...
            if (isConnected)
            {
                _Dbtask.ExecuteNonQueryAzureServer_SP("InsertTransactionReceipt", objArg);
            }
            else
            {
        ...
        {
            string Specific;
            Specific = _Dbtask.ExecuteScalar("select " + FiledName + " from tbltransactionreceipt where ReptCode='" + VNN + "'  and RecptType='" + vtype + "'  ");
            return Specific;
        }
        public void PVnoAzure(string Account, string Vtype)
        {
            string TempVno = _Dbtask.ExecuteScalarAzureServer("SELECT     { fn IFNULL(MAX(ReptCode), 0) } AS Expr1  FROM TbltransactionReceipt where RecptType='" + Vtype + "' and LedcodeDr='" + Account + "'");
            pvno = _Dbtask.ExecuteScalarAzureServer("select pvno from TbltransactionReceipt where ReptCode='" + TempVno + "' and RecptType='" + Vtype + "' and LedcodeDr='" + Account + "'");
            VNoStr = _Dbtask.ExecuteScalarAzureServer("select vno from TbltransactionReceipt where ReptCode='" + TempVno + "' and RecptType='" + Vtype + "' and LedcodeDr='" + Account + "'");
            if (VNoStr == "")
            {

                if (Vtype == "PI")
                {
                    VNoStr = _Dbtask.znllString(_Dbtask.ExecuteScalarAzureServer("select PARAMVALUE FROM TBLPARAMLIST WHERE PARAMNAME='MAXOFPI' "));
                }
                else
                {
                    VNoStr = "1";
...
// "   ORDER BY tblbusinessissue.Issuedate DESC  ";
        ...
        public void SRVnoAzure(string Account)
        {
            string TempVno = _Dbtask.ExecuteScalarAzureServer("SELECT     { fn IFNULL(MAX(ReptCode), 0) } AS Expr1  FROM TbltransactionReceipt where RecptType='SR' and LedcodeDr='" + Account + "'");
            pvno = _Dbtask.ExecuteScalarAzureServer("select pvno from TbltransactionReceipt where ReptCode='" + TempVno + "'  and RecptType='SR' and LedcodeDr='" + Account + "' ");
            VNoStr = _Dbtask.ExecuteScalarAzureServer("select vno from TbltransactionReceipt where ReptCode='" + TempVno + "' and  RecptType='SR' and LedcodeDr='" + Account + "'");
            if (VNoStr == "")
            {
                VNoStr = "1";
            }
        ...
        public string IdPurchaseFuAzure(string PurchaseAccount, string Vtype)
        {
            IdPurchase = Convert.ToInt64(Generalfunction.getnextidConditionAzure("ReptCode", "TblTransactionReceipt", " where RecptType='" + Vtype + "' and LedcodeDr='" + PurchaseAccount + "'"));
            return IdPurchase.ToString();
        }
        ...
        public string IdSRFuAzure(string PurchaseAccount)
        {
            IdPurchase = Convert.ToInt64(Generalfunction.getnextidCondition("ReptCode", "TblTransactionReceipt", " where RecptType='SR' and LedcodeDr='" + PurchaseAccount + "'"));
            return IdPurchase.ToString();
        }
        ...
        {
            AMTDb = _Dbtask.znullDouble(_Dbtask.ExecuteScalarAzureServer("select isnull(sum(" + Field + "),0) from Tbltransactionreceipt where  RecptType='" + VType + "' " + Where + ""));
            return AMTDb;
        }
        ...
        public double TottalAmountOfTypewthoutcrAzure(string Field, string VType, string Where)
        {
            AMTDb = _Dbtask.znullDouble(_Dbtask.ExecuteScalarAzureServer("select isnull(sum(" + Field + "),0) from Tbltransactionreceipt where  RecptType='" + VType + "' " + Where + "  and mpayment!='1' "));
            return AMTDb;
        }
        ...
        public double TottalAmountOfTypewherequeryAzure(string Field, string VType, string Where)
        {
            AMTDb = _Dbtask.znullDouble(_Dbtask.ExecuteScalarAzureServer("select isnull(sum(" + Field + "),0) from Tbltransactionreceipt where  RecptType='" + VType + "' " + Where + "   "));
            return AMTDb;
        }
