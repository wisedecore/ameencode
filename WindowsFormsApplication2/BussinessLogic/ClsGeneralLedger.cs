...
namespace WindowsFormsApplication2
{
    class ClsGeneralLedger
    {
            ...
            object[,] objArg = new object[20, 2]
        {
            ...
            _Dbtask.ExecuteNonQueryAzureServer_SP("InsertGeneralLedger", objArg);
            return;
        }
        public void InsertGeneralLedgerAzureServer()
        {
...
//_Dbtask.ExecuteNonQueryAzureServer_SP("InsertGeneralLedger", objArg);
        ...
        public double cashtobankamtPaymentAzure(string wheredatee, string TYPE)
        {
            double lastamt = 0; double totamtt = 0;
            if (TYPE == "C2B")
            {
                Ds = _Dbtask.ExecuteQueryAzureServer("select distinct vno from tblgeneralledger where vtype='P' AND " + wheredatee + " AND purticulars!='Two type paymentSales' and CRamt>0");
            }
            ...
            if (Ds.Tables[0].Rows.Count > 0)
            {
                for (int k = 0; k < Ds.Tables[0].Rows.Count; k++)
                {
                    ...
                    if (TYPE == "B2C")
                    {
                        DSw = _Dbtask.ExecuteQueryAzureServer("select * from tblgeneralledger where vno='" + tvno + "' and ledcode in('1') and vtype='P' and  CRAMT>0");
                    }
                    if (TYPE == "C2B")
                    {
                        DSw = _Dbtask.ExecuteQueryAzureServer("select * from tblgeneralledger where vno='" + tvno + "' and ledcode in('28') and vtype='P' and CRAMT>0");
                    }
                    ...
                    if (DSw.Tables[0].Rows.Count > 0)
                    {
                        for (int i = 0; i < DSw.Tables[0].Rows.Count; i++)
                        {
                            ...
                            if (leddcode == "1" || leddcode == "28")
                            {
                                if (leddcode == "1")
                                {
                                    amt = _Dbtask.znlldoubletoobject(_Dbtask.ExecuteScalarAzureServer("select sum(dbamt - cramt) from tblgeneralledger where vtype='P' and vno='" + tvno + "' and ledcode='" + leddcode + "'"));
                                    if (amt < 0)
                                    {
                                        amt = amt * (-1);
                                    }
                                    ...
                                    leddcode2 = "";
                                    leddcode2 = _Dbtask.znllString(_Dbtask.ExecuteScalarAzureServer("select ledcode from tblgeneralledger where vtype='P' and vno='" + tvno + "' and ledcode!='" + leddcode + "' and dbamt>0 "));
                                    if (leddcode2 == "28")
                                    {
                                        //amt2 = _Dbtask.znlldoubletoobject(_Dbtask.ExecuteScalar("select sum(dbamt - cramt) from tblgeneralledger where vtype='R' and vno='" + tvno + "' and ledcode='" + leddcode2 + "'"));
                                        //if (amt2 < 0)
                                ...
                                if (leddcode == "28")
                                {
                                    amt = _Dbtask.znlldoubletoobject(_Dbtask.ExecuteScalarAzureServer("select sum(dbamt)-sum(cramt) from tblgeneralledger where vtype='P' and vno='" + tvno + "' and ledcode='" + leddcode + "'"));
                                    leddcode2 = "";
                                    leddcode2 = _Dbtask.znllString(_Dbtask.ExecuteScalar("select ledcode from tblgeneralledger where vtype='P' and vno='" + tvno + "' and ledcode!='" + leddcode + "' and dbamt>0"));
                                    if (leddcode2 == "1")
                                    {
                                        amt2 = _Dbtask.znlldoubletoobject(_Dbtask.ExecuteScalarAzureServer("select sum(dbamt)-sum(cramt) from tblgeneralledger where vtype='P' and vno='" + tvno + "' and ledcode='" + leddcode2 + "'"));
                                        //totamtt = 0; amt3 = 0;
                                        amt3 = amt + amt3;
                                        totamtt = totamtt + amt3;
                                    }
        ...
        public double cashtobankamtReceiptAzure(string wheredatee, string TYPE)
        {
            double lastamt = 0; double totamtt = 0;
            if (TYPE == "C2B")
            {
                Ds = _Dbtask.ExecuteQueryAzureServer("select distinct vno from tblgeneralledger where vtype='R' AND " + wheredatee + " AND purticulars!='Two type paymentSales' and dbamt>0");
            }
            ...
            if (Ds.Tables[0].Rows.Count > 0)
            {
                for (int k = 0; k < Ds.Tables[0].Rows.Count; k++)
                {
                    ...
                    if (TYPE == "B2C")
                    {
                        DSw = _Dbtask.ExecuteQueryAzureServer("select * from tblgeneralledger where vno='" + tvno + "' and ledcode in('1') and vtype='R' and  dbamt>0");
                    }
                    if (TYPE == "C2B")
                    {
                        DSw = _Dbtask.ExecuteQueryAzureServer("select * from tblgeneralledger where vno='" + tvno + "' and ledcode in('28') and vtype='R' and dbamt>0");
                    }
                    ...
                    if (DSw.Tables[0].Rows.Count > 0)
                    {
                        for (int i = 0; i < DSw.Tables[0].Rows.Count; i++)
                        {
                            ...
                            if (leddcode == "1" || leddcode == "28")
                            {
                                if (leddcode == "1")
                                {
                                    amt = _Dbtask.znlldoubletoobject(_Dbtask.ExecuteScalarAzureServer("select sum(dbamt - cramt) from tblgeneralledger where vtype='R' and vno='" + tvno + "' and ledcode='" + leddcode + "'"));
                                    if (amt < 0)
                                    {
                                        amt = amt * (-1);
                                    }
                                    ...
                                    leddcode2 = "";
                                    leddcode2 = _Dbtask.znllString(_Dbtask.ExecuteScalarAzureServer("select ledcode from tblgeneralledger where vtype='R' and vno='" + tvno + "' and ledcode!='" + leddcode + "' and cramt>0 "));
                                    if (leddcode2 == "28")
                                    {
                                        //amt2 = _Dbtask.znlldoubletoobject(_Dbtask.ExecuteScalar("select sum(dbamt - cramt) from tblgeneralledger where vtype='R' and vno='" + tvno + "' and ledcode='" + leddcode2 + "'"));
                                        //if (amt2 < 0)
                                ...
                                if (leddcode == "28")
                                {
                                    amt = _Dbtask.znlldoubletoobject(_Dbtask.ExecuteScalarAzureServer("select sum(dbamt)-sum(cramt) from tblgeneralledger where vtype='R' and vno='" + tvno + "' and ledcode='" + leddcode + "'"));
                                    leddcode2 = "";
                                    leddcode2 = _Dbtask.znllString(_Dbtask.ExecuteScalarAzureServer("select ledcode from tblgeneralledger where vtype='R' and vno='" + tvno + "' and ledcode!='" + leddcode + "' and cramt>0"));
                                    if (leddcode2 == "1")
                                    {
                                        amt2 = _Dbtask.znlldoubletoobject(_Dbtask.ExecuteScalarAzureServer("select sum(dbamt)-sum(cramt) from tblgeneralledger where vtype='R' and vno='" + tvno + "' and ledcode='" + leddcode2 + "'"));
                                        //totamtt = 0; amt3 = 0;
                                        amt3 = amt + amt3;
                                        totamtt = totamtt + amt3;
                                    }
        ...
        public void IdGeneralLedgerAzure(string where)
        {
            VnoStr = Generalfunction.getnextidConditionforgeneralledgerAzure("vno", "Tblgeneralledger", where);
        }
        ...
        {
            double Tottal = _Dbtask.znullDouble(_Dbtask.ExecuteScalarAzureServer("select sum(" + Field + ") from Tblgeneralledger where  VType='" + VType + "' " + Where + ""));
            return Tottal;
        }
        ...
        public double getopeningbalanceAzure(string ledcode)
        {
            double opbalance = _Dbtask.znlldoubletoobject(_Dbtask.ExecuteScalarAzureServer("select dbamt-cramt from tblgeneralledger where ledcode = '" + ledcode + "' and vtype = 'ob'"));
            return opbalance;
        }
        ...
        public bool CountLedgerTRansactionAzure(string where)
        {
            Ds = _Dbtask.ExecuteQueryAzureServer("select * from tblgeneralledger " + where + "");
            if (Ds.Tables[0].Rows.Count > 0)
            {
                return true;
            }
