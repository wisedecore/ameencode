...

    {
        ...
        private void SetOrderData(Graphics g)
        {
           ...
           // g.DrawString(" /  " + Billno , VerdanaBold, BlackBrush, 750, CurrentY);
            ...
            if (isarbicdotprint == true)
            {
                ...
                g.DrawString(cusmob, VerdanaBold, BlackBrush, 100, 228);
                g.DrawString(BillDate.ToString("dd/MM/yyyy"), VerdanaBold, BlackBrush, 700, 190 , Str);
                if (FormType == "     SALES RETURN")
                {
                ...
                g.DrawString(Billno, VerdanaBold, BlackBrush, 300, 190,Str);



                g.DrawString(CName, VerdanaBold, BlackBrush, 640,228, Str);
                g.DrawString(CTin, Verdananormal, BlackBrush, 100, 263);
                ...
                if (_Dbtask.znllString(MainGrid.Rows[i].Tag) == "1")
                {
                    if (TempRowcounting < RecordsPerPage)
                    {
                        if (isarbicdotprint == true)
                        {
                            ...
                            g.DrawString(FieldValue, Verdananormal, BlackBrush, 15, CurrentY);
                            ...
                            if (FieldValue!= "")
                            {
                            g.DrawString(FieldValue, Verdananormal, BlackBrush,380 , CurrentY+10,Str);
                            }
                       ...
                       // if (SSlnotracking == true)
                        ...
                        if (InvSubTitle5 != "DELIVERY NOTE INVOICE")
                        {
                            if(isarbicdotprint==true)
                            {
                                ...
                                g.DrawString(FieldValue, Verdananormal, BlackBrush, 765, CurrentY,Str);
           ...
           "Fifty", "Sixty", "Seventy", "Eighty", "Ninety" };
            ...
            public String ConvertAmount(double amount)
            {
                try
                {
                    ...
                    ClsinvlaserThree Frms = new ClsinvlaserThree();
        ...
        public void VatPrinting(Graphics g)
        {
            ...
            if (Convert.ToDouble(TDiscount) > 0)
            {
                if (isarbicdotprint == true)
                {
                   // g.DrawString(" خصم", ArialnormalItalic, BlackBrush, 300, CurrentY,Str);
                    ...
                    g.DrawString(_Dbtask.SetintoDecimalpoint(_Dbtask.znlldoubletoobject(TDiscount)).ToString(), Arialboldbottm, BlackBrush, 680, 955);
            ...
            if(isarbicdotprint==true)
            {
                ...
                g.DrawString(_Dbtask.SetintoDecimalpoint(TotalTaxable).ToString(), Arialboldbottm, BlackBrush, 680, 925);
                g.DrawString(_Dbtask.SetintoDecimalpoint(TotalTaxAmount).ToString(), Arialboldbottm, BlackBrush, 680, 990);
        ...
        private void SetInvoiceTotal(Graphics g)
        {
            ...
            if (FormType != "DELIVERY NOTE")
            {
                ...
                if(isarbicdotprint==true)
                {
                    g.DrawString(" " + AmountinWords + " only", AmountInwordsfont3, BlackBrush, 18, 960);
                    g.DrawString(_Dbtask.SetintoDecimalpoint(GrandTotal), Arialboldbottm, BlackBrush, 680, 1024);
