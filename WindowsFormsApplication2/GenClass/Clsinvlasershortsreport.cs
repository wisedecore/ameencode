...
namespace WindowsFormsApplication2
{
    class Clsinvlasershortsreport
    {
        ...
        private void SetInvoiceData(Graphics g, System.Drawing.Printing.PrintPageEventArgs e)
        {
            ...
            int xRate;
            if (FormType == "SalesDay" || FormType == "Purchase")
            {
                RecordsPerPage = 45;
                CurrentY = 130;
                ...
                xQty = xProductName + (int)g.MeasureString("Note", VerdanaBold).Width + 50;
                g.DrawString("Party", VerdanaBold, BlackBrush, 230, CurrentY);      //Qty
                
                xQty = xProductName + (int)g.MeasureString("Note", VerdanaBold).Width + 50;
                g.DrawString("QTY", VerdanaBold, BlackBrush, 442, CurrentY);      //Qty

                xQty = xProductName + (int)g.MeasureString("Note", VerdanaBold).Width + 50;
                g.DrawString("Freeqty", VerdanaBold, BlackBrush, 525, CurrentY);      //Qty

                xQty = xProductName + (int)g.MeasureString("Note", VerdanaBold).Width + 50;
                g.DrawString("Tax", VerdanaBold, BlackBrush, 610, CurrentY);      //Qty
             ...
             for (int i = TRowcounting; i <= MainGrid.Rows.Count - 1; i++)
              {
                  ...
                  if (_Dbtask.znllString(MainGrid.Rows[i].Cells["clndate"].Value) != "" && _Dbtask.znlldoubletoobject(MainGrid.Rows[i].Cells["clnnetamount"].Value) > 0)
                  {
                      if (TempRowcounting < RecordsPerPage)
                      {
                          ...
                          FieldValue = _Dbtask.znllString(MainGrid.Rows[i].Cells["clnqty"].Value);
                          qty = _Dbtask.znullDouble(FieldValue);
                          g.DrawString(_Dbtask.SetintoDecimalpoint(qty), Verdananormal, BlackBrush, 430, CurrentY);

                          netqty = netqty + qty;

                          FieldValue = _Dbtask.znllString(MainGrid.Rows[i].Cells["clnfreeqty"].Value);
                          ...
                          //else
                          //{
                              FieldValue = _Dbtask.znllString(MainGrid.Rows[i].Cells["clntaxamt"].Value);
                          //}
                          amt = _Dbtask.znullDouble(FieldValue);
                          //FieldValue = _Dbtask.SetintoDecimalpoint(amt)//.ToString();
                          g.DrawString(_Dbtask.SetintoDecimalpoint(amt), Verdananormal, BlackBrush, 650, CurrentY, Str);

                          FieldValue = _Dbtask.znllString(MainGrid.Rows[i].Cells["clnnetamount"].Value);
                    ...
                    if (TempRowcounting < RecordsPerPage)
                    {
                        ...
                        if (MainGrid.Rows[i].Cells["clndate"].Value == "Ledger")
                        {
                            ...
                            if (FieldValue != "")
                            {
                                ...
                                g.DrawString("Time Period : " + Timeperiod, VerdanaBold, BlackBrush, 100, CurrentY - 80);
                            }
                        }
            ...
            if (InvSubTitle5 != "      ESTIMATE" && FormType != "SalesDay" && FormType != "Sales" && FormType != "Purchase")
            {
            ...
            else
            {
                ...
                if (FormType != "SalesDay" && FormType != "Sales" && FormType != "Purchase")
                {
            ...
            {
            g.DrawString("Total", Verdananormal, BlackBrush, 250, CurrentY);
            g.DrawString(_Dbtask.znllString( netqty), Verdananormal, BlackBrush, 450, CurrentY,Str);
            g.DrawString(_Dbtask.znllString(netamt), Verdananormal, BlackBrush, 750, CurrentY, Str);
            }
            // Set Invoice Table:
            ...
            CurrentY = CurrentY + InvoiceFontHeight + 2;
            string ItemNote;
            if (FormType != "SalesDay" && FormType != "AccountReport" && FormType != "Sales" && FormType != "Purchase")
            {
                for (int i = TRowcounting; i < MainGrid.Rows.Count - 1; i++)
                {
           ...
           "Fifty", "Sixty", "Seventy", "Eighty", "Ninety" };
            ...
            public String ConvertAmount(double amount)
            {
                try
                {
                    ...
                    Clsinvlasershortsreport Frms = new Clsinvlasershortsreport();
