...
namespace WindowsFormsApplication2
{
    class ClsBalancesheet
    {
     ...
     //           g.DrawString(InvSubTitle3, Arialbold, BlackBrush, xInvSubTitle1-20, CurrentY);
            ...
            if (FormType == "Balancesheet" && head == false)
            {
                CurrentY = 10;
            ...
            if (FormType == "Balancesheet")
            {
                ...
                g.DrawString("Balancesheet", gothicfnt, BlackBrush, 300, CurrentY);
                CurrentY = CurrentY + 23;
       ...
       //     CurrentY = CurrentY + InvSubTitleHeight + 8;
        ...
        {// Set Invoice Table:
          ...
          //\ g.DrawString("SlNo", Verdananormal, BlackBrush, xSlno, CurrentY);
            ...
            {
                g.DrawString("Assets", VerdanaBold2, BlackBrush, 20, CurrentY);
                g.DrawString("Amount", VerdanaBold2, BlackBrush, 340, CurrentY,Str );
                g.DrawString("Laibilities", VerdanaBold2, BlackBrush, 350, CurrentY);
           ...
           //CurrentY = CurrentY + InvoiceFontHeight + 2;
            ...
            if (FormType == "Balancesheet")
            {
                    ...
                    for (int i = TRowcounting; i <= MainGrid.Rows.Count - 1; i++)
                    {
                        ...
                        if (_dbtask.znllString(MainGrid.Rows[i].Cells["clnaamount"].Value) != "" || _dbtask.znllString(MainGrid.Rows[i].Cells["clnlamount"].Value) != "")
                        {
                        ...
                        if (TempRowcounting < RecordsPerPage)
                        {
                            ...
                            FieldValue = _dbtask.znllString(MainGrid.Rows[i].Cells["clnassets"].Value);
                            e.Graphics.DrawString(FieldValue, Verdananormalbld, BlackBrush, 20, CurrentY);

                            FieldValue = _dbtask.znllString(MainGrid.Rows[i].Cells["clnaamount"].Value);
                            e.Graphics.DrawString(FieldValue, Verdananormalbld, BlackBrush,340, CurrentY,Str);

                            FieldValue = _dbtask.znllString(MainGrid.Rows[i].Cells["clnlaibilities"].Value);
                            e.Graphics.DrawString(FieldValue, Verdananormalbld, BlackBrush, 350, CurrentY);

                            FieldValue = _dbtask.znllString(MainGrid.Rows[i].Cells["clnlamount"].Value);
           ...
           "Fifty", "Sixty", "Seventy", "Eighty", "Ninety" };
            ...
            public String ConvertAmount(double amount)
            {
                try
                {
                    ...
                    ClsBalancesheet Frms = new ClsBalancesheet();
