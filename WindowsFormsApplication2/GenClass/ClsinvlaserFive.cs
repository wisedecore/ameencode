...
namespace WindowsFormsApplication2
{
    class ClsinvlaserFive
    {
        ...
        public bool Preview; public int HF = 0;
        /***************/

        public int amtwrd = 0;
        public string projctname= "";
        public string ponum = "";
        public string deliverynot= "";
        ...
        private Font Verdananormalarrb = new Font("Calibri", 8, FontStyle.Regular);


        private Font Verdansign = new Font("Calibri", 8, FontStyle.Italic);
        private Font Verdasmaallll = new Font("Calibri", 8, FontStyle.Bold);
        private Font Verdansignnew = new Font("Calibri", 7, FontStyle.Regular);
        private Font Verdananormalsmll = new Font("Calibri", 8, FontStyle.Bold);

        private Font Verdananormaleight = new Font("Calibri", 8, FontStyle.Bold);
        private Font Verdananormaleghtregu = new Font("Calibri", 8, FontStyle.Regular);


        private Font Verdanaitalic = new Font("Verdana", 6, FontStyle.Regular);
        //Declaration Font
        ...
        private void SetInvoiceHead(Graphics g)
        {
            ...
            g.DrawRectangle(selpen, 10, CurrentY, 770, 20);
           // StrTaxinvoiceHeader = "";
            CurrentY = CurrentY + 2;//142
            if (FormType == "     SALES QUATATION")
            {
                StrTaxinvoiceHeader = " (Sale Quatation    - تسعير المبيعات )";//, VerdanaBold, BlackBrush, 480, CurrentY);//142
            }
            if (FormType == "     SALES ORDER")
            {
                StrTaxinvoiceHeader = "(Sale Order   - طلب المبيعات )";//, VerdanaBold, BlackBrush, 480, CurrentY);//142
            }
            if (FormType == "     SALES RETURN")
            {
                StrTaxinvoiceHeader = "(Sales Return- عرض السعر )";//, VerdanaBold, BlackBrush, 550, CurrentY);//142
            }
            if (FormType == "     SALES")
            {
                StrTaxinvoiceHeader = StrTaxinvoiceHeader;// "فاتورة ضريبية مبسطة" + "  " + "Simplified Tax Invoice";
            }
            if (FormType == "     DELIVERY NOTE")
            {
                StrTaxinvoiceHeader = " DELIVERY NOTE";
            }
            //if (FormType == "     SALES RETURN")
            //{
            //    g.DrawString("(Saler Return-مرتجع فاتورة)", VerdanaBold, BlackBrush, 550, CurrentY);
            //}
            //StrTaxinvoiceHeader = "فاتورة ضريبية مبسطة" + "  " + "Simplified Tax Invoice";
            g.DrawString(StrTaxinvoiceHeader, VerdanaBold, BlackBrush, 300, CurrentY);
            g.DrawString(ModeofPayment, VerdanaBold, BlackBrush, 20, CurrentY);

            CurrentY = CurrentY + 26;//168
            g.DrawString("Invoice no.", Verdananormal, BlackBrush, 10, CurrentY);
            g.DrawString("Mobile", Verdananormal, BlackBrush, 381, CurrentY);

            CurrentY = CurrentY + 2;//170
            g.DrawRectangle(selpen2, 10, CurrentY, 370, 60);
            g.DrawRectangle(selpen2, 380, CurrentY, 400, 60);
            g.DrawLine(new Pen(Brushes.Black, 1), 150, CurrentY, 150, CurrentY+60);
            g.DrawLine(new Pen(Brushes.Black, 1), 525, CurrentY, 525, CurrentY + 60);

            CurrentY = CurrentY + 2;//172
            g.DrawString(cusmob, VerdanaBold, BlackBrush, 540, CurrentY);//CName
            g.DrawString(  pvnoval+" "+Billno, VerdanaBold, BlackBrush, 157, CurrentY);


            CurrentY = CurrentY + 10;//182
            g.DrawString("رقم الفاتورة", Verdananormal, BlackBrush, 10, CurrentY);
            g.DrawString(_Gen.FuReturnArabic("Mobile"), Verdananormal, BlackBrush, 381, CurrentY);

            CurrentY = CurrentY + 16;//198

            g.DrawLine(new Pen(Brushes.Black, 1), 10, CurrentY, 372, CurrentY);

            g.DrawLine(new Pen(Brushes.Black, 1), 380, CurrentY, 772, CurrentY);
            g.DrawLine(new Pen(Brushes.Black, 1), 10, CurrentY, 780, CurrentY);

                CurrentY = CurrentY + 2;//200
                g.DrawString(BillDate.ToString("dd/MM/yyyy"), Verdananormalsmll, BlackBrush, 150, CurrentY);//+ "  Time:" + BillDate.ToString("hh:mm:ss")
            //g.DrawString(cusmob, VerdanaBold, BlackBrush, 540, 172);//CName
            g.DrawString(CTin, VerdanaBold, BlackBrush, 540, CurrentY);
            g.DrawString("Date.", Verdananormalsmll, BlackBrush, 10, CurrentY);
            g.DrawString("Customer VAT", Verdananormal, BlackBrush, 381, CurrentY);

            g.DrawString("تاريخ", Verdananormalsmll, BlackBrush, 50, CurrentY);
            CurrentY = CurrentY + 14;//214
            g.DrawString("DueDate", Verdananormalsmll, BlackBrush, 10, CurrentY);
            g.DrawString(duedate, Verdananormalsmll, BlackBrush, 150, CurrentY);
            
            
            g.DrawString(_Gen.FuReturnArabic("Vat Cus"), Verdananormal, BlackBrush, 381, CurrentY);

            //CurrentY = CurrentY +20;//240
            //g.DrawRectangle(selpen2, 10, CurrentY, 770, 100);
            

            //g.DrawString("Project : "+projctname, Verdananormal, BlackBrush,10, CurrentY);
            //CurrentY = CurrentY + 20;
            //g.DrawLine(new Pen(Brushes.Black, 1), 10, CurrentY, 775, CurrentY);
            //g.DrawString("PO No. : "+ponum, Verdananormal, BlackBrush, 10, CurrentY);

            //g.DrawString("MRF/PR No : "+mrfprnum , Verdananormal, BlackBrush, 381, CurrentY);

            //g.DrawLine(new Pen(Brushes.Black, 1), 370, CurrentY, 370, CurrentY + 30);

            //CurrentY = CurrentY + 30;
            //g.DrawLine(new Pen(Brushes.Black, 1), 10, CurrentY, 775, CurrentY);

            //g.DrawString("Delivery Note : "+deliverynot, Verdananormal, BlackBrush, 10, CurrentY);

            //CurrentY = CurrentY + 25;
            //g.DrawLine(new Pen(Brushes.Black, 1), 10, CurrentY, 775, CurrentY);

            //g.DrawString("Attention : "+attention, Verdananormal, BlackBrush, 10, CurrentY);




            CurrentY = CurrentY + 20;//240
            g.DrawRectangle(selpen2, 10, CurrentY, 770, 70);


            CurrentY = CurrentY + 2;//242
            g.DrawString(CName, Verdasmaallll, BlackBrush, 10, CurrentY);
            CurrentY = CurrentY + 16;

            string SECLANG = "";
            SECLANG = _Dbtask.znllString(_Accountledger.GetspecifField("customercode", Lid));
            g.DrawString(SECLANG, Verdasmaallll, BlackBrush, 750, CurrentY, Str);
            g.DrawString("Customer " + _Gen.FuReturnArabic("Cus Name") + " : ", Verdananormal, BlackBrush, 10, CurrentY);

            CurrentY = CurrentY + 26;//260

            //CurrentY = CurrentY + 2;//242
            //g.DrawString(CName, VerdanaBold, BlackBrush, 150, CurrentY);
            //CurrentY = CurrentY + 16;

            //string SECLANG = "";
            //SECLANG = _Accountledger.GetspecifField("customercode", Lid);
            //g.DrawString(SECLANG, Verdananormal, BlackBrush, 750, CurrentY,Str);
            //g.DrawString("Customer " + _Gen.FuReturnArabic("Cus Name") + " : ", Verdananormal, BlackBrush, 10, CurrentY);
            ...
            g.DrawString(Address.Replace("\n", ""), VerdanaBold, BlackBrush, 150, CurrentY);
            g.DrawLine(new Pen(Brushes.Black, 1), 10, CurrentY, 780, CurrentY);
            //g.DrawString(cusmob, VerdanaBold, BlackBrush, 730, CurrentY, Str);


            CurrentY = CurrentY + 5;//265
            g.DrawString("Address " + _Gen.FuReturnArabic("Address"), Verdananormal, BlackBrush, 10, CurrentY);
            ...
            g.DrawString("Project : " + projctname, Verdananormalsmll, BlackBrush, 10, CurrentY+5);
            //g.DrawString("MRF/PR No:" + mrfprnum, Verdananormalsmll, BlackBrush, 500, CurrentY + 5);
           
            
            CurrentY = CurrentY + 20;
          
            g.DrawLine(new Pen(Brushes.Black, 1), 10, CurrentY, 780, CurrentY);
            g.DrawString("PO No. : " + ponum, Verdananormalsmll, BlackBrush, 10, CurrentY + 5);
            g.DrawString("Delivery Note No. : " + deliverynot, Verdananormalsmll, BlackBrush, 270, CurrentY + 5);

           

            g.DrawLine(new Pen(Brushes.Black, 1), 270, CurrentY, 270, CurrentY + 30);

            CurrentY = CurrentY + 30;
            g.DrawLine(new Pen(Brushes.Black, 1), 10, CurrentY, 780, CurrentY);
            g.DrawString("Attention : " + attention, Verdananormalsmll, BlackBrush, 10, CurrentY+5);
            g.DrawString("MRF/PR No:" + mrfprnum, Verdananormalsmll, BlackBrush, 500, CurrentY + 5);
            g.DrawLine(new Pen(Brushes.Black, 1), 500, CurrentY, 500, CurrentY + 20);
            ...
            g.DrawRectangle(selpen, 10, CurrentY, 770, (530 - toplasr));
            g.DrawLine(new Pen(Brushes.Black, 1), 50, CurrentY, 50, CurrentY + (530 - toplasr));
            g.DrawLine(new Pen(Brushes.Black, 1), 400, CurrentY, 400, CurrentY + (530 - toplasr));
            g.DrawLine(new Pen(Brushes.Black, 1), 450, CurrentY, 450, CurrentY + (530 - toplasr));
            g.DrawLine(new Pen(Brushes.Black, 1), 500, CurrentY, 500, CurrentY + (530 - toplasr));
            g.DrawLine(new Pen(Brushes.Black, 1), 565, CurrentY, 565, CurrentY + (530 - toplasr));
            g.DrawLine(new Pen(Brushes.Black, 1), 630, CurrentY, 630, CurrentY + (530 - toplasr));
            //g.DrawLine(new Pen(Brushes.Gray, 1), 650, 290, 650, 725);
            g.DrawLine(new Pen(Brushes.Black, 1), 685, CurrentY, 685, CurrentY + (530 - toplasr));


            CurrentY = CurrentY + 10;//300
            g.DrawString("Slno", Verdananormal, BlackBrush, 10, CurrentY);
            g.DrawString("Itemname", Verdananormal, BlackBrush, 200, CurrentY);
            g.DrawString("Qty", Verdananormal, BlackBrush, 410, CurrentY);
            g.DrawString("Unit", Verdananormal, BlackBrush, 455, CurrentY);
            g.DrawString("Unitprice", Verdananormal, BlackBrush, 505, CurrentY);
            g.DrawString("Gross", Verdananormal, BlackBrush, 578, CurrentY);
            g.DrawString("Tax15%", Verdananormal, BlackBrush, 630, CurrentY);
            g.DrawString("Amount", Verdananormal, BlackBrush, 700, CurrentY);
           // g.DrawRectangle(selpen2, 380, 170, 380, 60);

            CurrentY = CurrentY + 12;//312
            g.DrawString(_Gen.FuReturnArabic("Slno"), Verdananormal, BlackBrush, 10, CurrentY);

            g.DrawString(_Gen.FuReturnArabic("Item Name"), Verdananormal, BlackBrush, 200, CurrentY);

            g.DrawString(_Gen.FuReturnArabic("Qty"), Verdananormal, BlackBrush, 410, CurrentY);

            g.DrawString(_Gen.FuReturnArabic("Unit"), Verdananormal, BlackBrush, 455, CurrentY);

            g.DrawString(_Gen.FuReturnArabic("Unit Rate"), Verdananormal, BlackBrush, 505, CurrentY);

            g.DrawString(_Gen.FuReturnArabic("Gross"), Verdananormal, BlackBrush, 578, CurrentY);

            g.DrawString(_Gen.FuReturnArabic("Tax 15%"), Verdananormal, BlackBrush, 630, CurrentY);

            g.DrawString(_Gen.FuReturnArabic("Amount"), Verdananormal, BlackBrush, 700, CurrentY);
            ...
            if (HF == 0 || HF == 2)
            {
                ...
                //if (System.IO.File.Exists(InvImagehead) && FormType != "ESTIMATE" && HeaderinPrint == true)
                if (System.IO.File.Exists(InvImagehead) && FormType != "ESTIMATE" )
                {
                    ...
                    ImageHeight = oInvImage.Height; // Get Image Height
                    ImageHeight = 0;
                    g.DrawImage(oInvImage, 10, 5, 770, 130);
                }
            }
           ...
           // if (System.IO.File.Exists(invimagefootr) && FormType != "ESTIMATE"&&HeaderinPrint==true)
            if (System.IO.File.Exists(invimagefootr) && FormType != "ESTIMATE" )
            {
                ...
                ImageHeight = oInvImage.Height; // Get Image Height
                ImageHeight = 0;
                g.DrawImage(oInvImage, 155, 1010, 624, 60);
            }
        ...
        {// Set Invoice Table:
           ...
           // CurrentY = CurrentY + 40;
                ...
                if (_Dbtask.znllString(MainGrid.Rows[i].Tag) == "1")
                {
                    if (TempRowcounting < RecordsPerPage)
                    {
                        if (isarbicdotprint == true)
                        {
                            ...
                            g.DrawString(FieldValue, Verdananormal, BlackBrush, 20, CurrentY);
                            ...
                            if (Plangwise == 2)
                            {
                                ...
                                if (FieldValue != "")
                                {
                                    g.DrawString(FieldValue, Verdananormaleight, BlackBrush, 50, CurrentY);
                                }
                                ...
                                if (FieldValue != "")
                                {
                                    ...
                                    g.DrawString(FieldValue, Verdananormaleight, BlackBrush, 390, CurrentY, Str);
                                }
                        ...
                        if (InvSubTitle5 != "DELIVERY NOTE INVOICE")
                        {
                            if (isarbicdotprint == true)
                            {
                                ...
                                FieldValue = _Dbtask.znllString(_Dbtask.znllString(MainGrid.Rows[i].Cells["ClnbaseU"].Value));
                                //FieldValue = 

                                FieldValue = Regex.Replace(FieldValue, "[0-9]", "");
                                FieldValue = Regex.Replace(FieldValue, "-", "");
                                g.DrawString(FieldValue, Verdananormal, BlackBrush, 469, CurrentY);
                                ...
                                g.DrawString(FieldValue, Verdananormal, BlackBrush, 556, CurrentY, Str);
            ...
            {
                if (ValidRecord > TempRowcounting)
                {
                    e.HasMorePages = true;
                    TempRowcounting = 0;
           ...
           "خمسون", "ستون", "سبعون", "ثمانون", "تسعين" };
            ...
            public String ConvertAmount(double amount)
            {
                try
                {
                    ...
                    ClsinvlaserFive Frms = new ClsinvlaserFive();
        ...
        public void PrintTotal(Graphics g)
        {
            ...
            if (InvSubTitle5 != "DELIVERY NOTE INVOICE")
            {
                ...
                g.DrawLine(new Pen(Brushes.Black, 1), 10, CurrentY-9, 780, CurrentY-9);


                g.DrawString("Total", Verdananormalarrb, BlackBrush, 150, CurrentY-5);
                g.DrawString(TotalQty, Verdananormalarrb, BlackBrush, 410, CurrentY - 5);//770/*For Qty*///350
                g.DrawString(_Dbtask.SetintoDecimalpoint(TotalTaxable), Verdananormalarrb, BlackBrush, 628, CurrentY - 5, Str);/*For Qty*///350
                ...
                g.DrawString(_Dbtask.SetintoDecimalpoint(TotalTaxAmount), Verdananormalarrb, BlackBrush, 680, CurrentY - 5, Str);/*For Tax*/
                g.DrawString(_Dbtask.SetintoDecimalpoint(TotalNetamount), Verdananormalarrb, BlackBrush, 755, CurrentY - 5, Str);/*For Amount*/

                CurrentY = CurrentY;
                CurrentY = CurrentY + 20;
                CurrentY = CurrentY + (800 - CurrentY);//800

                g.DrawRectangle(selpen, 10, CurrentY, 130, 90);
                g.DrawRectangle(selpen, 350, CurrentY, 432, 150);
                g.DrawLine(new Pen(Brushes.Black, 1), 600, CurrentY, 600, CurrentY + 150);
                //g.DrawLine(new Pen(Brushes.Black, 1), 10, CurrentY, 781, CurrentY);
                if (CommonClass._Menusettings.GetMnustatus("9000") == "1")
                {
                    setqrtwo(g);
                }

                CurrentY = CurrentY + 5;//805
                g.DrawString("Authorised Signatory ", Verdananormal, BlackBrush, 12, 805);

                CurrentY = CurrentY + 10;//815

                g.DrawString("Discount   " +  "", Verdananormal, BlackBrush, 370, CurrentY-12);
                g.DrawString( _Gen.FuReturnArabic("Discount") + "", Verdananormal, BlackBrush, 370, CurrentY+4);
                ...
                if (Convert.ToDouble(TDiscount) > 0)
                {
                    if (isarbicdotprint == true)
                    {
                        // g.DrawString(" خصم", ArialnormalItalic, BlackBrush, 300, CurrentY,Str);
                        xTotalValue = 750 - (int)g.MeasureString(TDiscount, ArialnormalItalic).Width;
                        g.DrawString(_Dbtask.SetintoDecimalpoint(_Dbtask.znlldoubletoobject(TDiscount)).ToString(), Arialboldbottm, BlackBrush, 750, CurrentY, Str);
                    }
                }

                CurrentY = CurrentY + 5;//820
                g.DrawString("المفوض بالتوقيع ", Verdananormal, BlackBrush, 12, CurrentY);

                CurrentY = CurrentY + 15;//835
                g.DrawLine(new Pen(Brushes.Black, 1), 350, 835, 781, CurrentY);

                CurrentY = CurrentY + 10;//845
                g.DrawString("Tot.Amount(exclusive vat) " +  "", Verdananormal, BlackBrush, 370, CurrentY-7);
                g.DrawString( _Gen.FuReturnArabic("Excl Vat") + "", Verdananormal, BlackBrush, 370, CurrentY+6);
                ...
                g.DrawLine(new Pen(Brushes.Black, 1), 350, CurrentY, 781, CurrentY);

                CurrentY = CurrentY + 9;//885

                g.DrawString("Total vat 15%   "  + "", Verdananormal, BlackBrush, 370, CurrentY-5);

                g.DrawString( _Gen.FuReturnArabic("Total Vat") + "", Verdananormal, BlackBrush, 370, CurrentY+6);
                ...
                if (isarbicdotprint == true)
                {
                    xTotalValue = 750 - (int)g.MeasureString(BillAmount, Arialbold).Width;
                    //g.DrawString(_Dbtask.SetintoDecimalpoint(TotalTaxable).ToString(), Arialboldbottm, BlackBrush, 750, CurrentY, Str);
                    g.DrawString(_Dbtask.SetintoDecimalpoint(TotalTaxAmount).ToString(), Arialboldbottm, BlackBrush, 750, CurrentY, Str);
                }
                ...
                g.DrawString("Total Amount   "  + "", Verdananormal, BlackBrush, 370, CurrentY-7);

                g.DrawString( _Gen.FuReturnArabic("Amount Total") + "", Verdananormal, BlackBrush, 370, CurrentY+8);



                CurrentY = CurrentY + 33;//960
                g.DrawRectangle(selpen2, 160, CurrentY+20, 624, 30);

                CurrentY = CurrentY + 5;//970
                g.DrawString("Grand total in words", Verdananormal, BlackBrush, 10, CurrentY);

                CurrentY = CurrentY + 20;//970
                g.DrawString("Receiver Name", Verdansign, BlackBrush, 780, CurrentY,Str);

                g.DrawString("& Sign", Verdansign, BlackBrush, 780, CurrentY + 12, Str);
        ...
        public void setqrtwo(Graphics g)
        {
            ...
            StrQrData = _Gen.FureturnQrValue(_Companymaster.GetspecifField("Sellersname"), _Companymaster.GetspecifField("TRN"), Billno, BillDate, Ttaxamount, BillAmount); options = new QrCodeEncodingOptions
            {
                ...
                //Height = 97,
                        ...
                        if(Clssecondphase.Strqrcodesecondphase!=""&& Clssecondphase.Strqrcodesecondphase != null)
            StrQrData = Clssecondphase.Strqrcodesecondphase;
            ...
            g.DrawImage(newbit, 150, CurrentY-3);
            ...
            if (FormType != "DELIVERY NOTE")
            {
                ...
                if (isarbicdotprint == true)
                {
                    ...
                    if (amtwrd == 1)
                    {
                        ...
                        g.DrawString(" " + AmountinWords + " only", AmountInwordsfont3, BlackBrush, 180, CurrentY+5);
                        //arabic
                        //g.DrawString(" " + AmountinWords + " فقط", Verdananormal, BlackBrush, 660, CurrentY, Str);//x180
                        //g.DrawString("   :  مبلغا وقدرة", Verdananormal, BlackBrush, (750), CurrentY, Str);//x180  (leftlase + 180)

                        CurrentY = CurrentY + 10;//985
                        g.DrawString(_Gen.FuReturnArabic("Amount in words"), Verdananormal, BlackBrush, 10, CurrentY);
                    ...
                    if (amtwrd == 0)
                    {
                       ...
                       AmountinWords = _wordar.ConvertAmount(Convert.ToDouble(BillAmount));
                        ...
                        g.DrawString(" " + AmountinWords + " فقط", Verdananormal, BlackBrush, 660, CurrentY+4, Str);//x180
                       g.DrawString("   :  مبلغا وقدرة", Verdananormal, BlackBrush, (750), CurrentY, Str);//x180  (leftlase + 180)
                    ...
                    g.DrawString(_Dbtask.SetintoDecimalpoint(GrandTotal), VerdanaBoldHeading, BlackBrush, 750, CurrentY, Str);
                    grandamtt = _Dbtask.SetintoDecimalpoint(GrandTotal).ToString();
                    int ImageHeight = 0;
                    string invimagesign;
                    invimagesign = Application.StartupPath + @"\Images\" + "Sign.jpeg";
                    // if (System.IO.File.Exists(invimagefootr) && FormType != "ESTIMATE"&&HeaderinPrint==true)
                    if (System.IO.File.Exists(invimagesign) && FormType != "ESTIMATE")
                    {
                        Bitmap oInvImage = new Bitmap(invimagesign);
                        // Set Image Left to center Image:
                        int xImage = InvoiceWidth - oInvImage.Width;
                        ImageHeight = oInvImage.Height; // Get Image Height
                        ImageHeight = 0;
                        //g.DrawImage(oInvImage, 60, 820, 90, 60);


                        g.DrawImage(oInvImage, 30, 838, 80, 50);
                    }
                    ...
                    if (System.IO.File.Exists(invimageseal) && FormType != "ESTIMATE")
                    {
                        ...
                        ImageHeight = oInvImage.Height; // Get Image Height
                        ImageHeight = 0;
                        g.DrawImage(oInvImage, 5, 920, 150, 150);
                    }
