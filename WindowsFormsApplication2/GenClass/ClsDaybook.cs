...
namespace WindowsFormsApplication2
{
    class ClsDaybook
    {
       ...
       // private Font InvoiceFont = new Font("Arial", 12, FontStyle.Regular);
        ...
        private Font Verdanasmalls = new Font("Verdana", 7, FontStyle.Regular);
     ...
     //           g.DrawString(InvSubTitle3, Arialbold, BlackBrush, xInvSubTitle1-20, CurrentY);
            ...
            if (FormType == "Daybook" && head == false)
            {
                CurrentY = 10;
            ...
            if (FormType == "Daybook")
            {
                ...
                //g.DrawString(InvTitle, Arialbold, BlackBrush, 300, 20);
               
                //g.DrawString( InvSubTitle1, VerdanaBold, BlackBrush, 300, 40);
                //g.DrawString("VAT : "+InvSubTitle3, VerdanaBold, BlackBrush, 300, 60);

                g.DrawString("Daybook", gothicfnt, BlackBrush, 300, CurrentY);
                CurrentY = CurrentY + 23;
       ...
       //     CurrentY = CurrentY + InvSubTitleHeight + 8;
        ...
        {// Set Invoice Table:
          ...
          //\ g.DrawString("SlNo", Verdananormal, BlackBrush, xSlno, CurrentY);
            int xProductName;
            int xRate;
            SolidBrush fillgry = new SolidBrush(Color.LightSteelBlue);
            Pen selcln = new Pen(Color.LightSteelBlue);
            ...
            {
                colustart = CurrentY+20;
                g.DrawString("Vno", VerdanaBold2, BlackBrush, 20, CurrentY);
                g.DrawString("Date", VerdanaBold2, BlackBrush, 60, CurrentY );
                g.DrawString("Type", VerdanaBold2, BlackBrush,180, CurrentY);
                //CurrentX = CurrentX + 10;
                g.DrawString("Purticulars", VerdanaBold2, BlackBrush, 280, CurrentY);
                CurrentX = CurrentX + 80;


                g.DrawString("Credit", VerdanaBold2, BlackBrush, 550, CurrentY, Str);
                CurrentX = CurrentX + 10;

                g.DrawString("Debit", VerdanaBold2, BlackBrush, 650, CurrentY, Str);
                CurrentX = CurrentX + 10;

                g.DrawString("Balance", VerdanaBold2, BlackBrush, 800, CurrentY,Str);
                CurrentX = CurrentX + 10;
                //+ 10;
          ...
          //  RowValidation(MainGrid);
           ...
           //CurrentY = CurrentY + InvoiceFontHeight + 2;
            string ItemNote;
            SolidBrush grycln = new SolidBrush(Color.Orange);
            Pen rowclr = new Pen(Color.Orange);
            ...
            if (FormType == "Daybook")
            {
                    ...
                    for (int i = TRowcounting; i <= MainGrid.Rows.Count - 1; i++)
                    {
                        ...
                        if (_dbtask.znllString(MainGrid.Rows[i].Cells["clncredit"].Value) != "" || _dbtask.znllString(MainGrid.Rows[i].Cells["clndebit"].Value) != "")
                        {
                     
                        if (TempRowcounting < RecordsPerPage)
                        {
                            // String.Format("{0:0.00}", _Dbtask.gridnul(MainGrid.Rows[i].Cells["clnqty"].Value));
                            //  FieldValue = _dbtask.znllString(MainGrid.Rows[i].Cells["clnvno"].Value);
                            //FieldValue = FieldValue.Substring(0, 11).ToString();
                            // e.Graphics.DrawString(FieldValue, VerdanaBoldHeading, BlackBrush, 30, CurrentY);
                            FieldValue = _dbtask.znllString(MainGrid.Rows[i].Cells["clnvno"].Value);
                            e.Graphics.DrawString(FieldValue, Verdananormalbld, BlackBrush, 20, CurrentY);

                            FieldValue = _dbtask.znllString(MainGrid.Rows[i].Cells["clndate"].Value);
                            if (FieldValue != "")
                            {
                                int leng = 0;
                                leng = FieldValue.Length;
                                e.Graphics.DrawString(FieldValue.Substring(0, 10), Verdanasmalls, BlackBrush, 60, CurrentY);
                                CurrentY = CurrentY + 15;
                                e.Graphics.DrawString(FieldValue.Substring(11, leng - 11), Verdanasmalls, BlackBrush, 60, CurrentY);
                            }
                            else
                            {
                                e.Graphics.DrawString(FieldValue, Verdanasmalls, BlackBrush, 60, CurrentY);
                            }
                            FieldValue = _dbtask.znllString(MainGrid.Rows[i].Cells["clnvtype"].Value);
                            e.Graphics.DrawString(FieldValue, Verdanasmalls, BlackBrush, 190, CurrentY);

                            FieldValue = _dbtask.znllString(MainGrid.Rows[i].Cells["clnpurticulars"].Value);

                            if (FieldValue.Length > 23)
                            {
                                e.Graphics.DrawString(FieldValue.Substring(0, 22), Verdanasmalls, BlackBrush, 270, CurrentY - 15);
                                e.Graphics.DrawString(FieldValue.Substring(22), Verdanasmalls, BlackBrush, 270, CurrentY);
                            }
                            else
                            {
                                e.Graphics.DrawString(FieldValue, Verdanasmalls, BlackBrush, 270, CurrentY);
                            }

                            //FieldValue = _dbtask.znllString(MainGrid.Rows[i].Cells["clnpayment"].Value);
                            //e.Graphics.DrawString(FieldValue, Verdananormalbld, BlackBrush, 450, CurrentY);



                            //FieldValue = String.Format("{0:0.00}", _Dbtask.gridnul(MainGrid.Rows[i].Cells["clnramount"].Value));
                            //e.Graphics.DrawString(FieldValue, Verdananormalbld, BlackBrush, 380, CurrentY,Str);

                            FieldValue = _dbtask.znllString(MainGrid.Rows[i].Cells["clndebit"].Value);
                            if (FieldValue == "Today Balance"||FieldValue == "TodayClosing Balance")
                            {
                                SolidBrush filloliv = new SolidBrush(Color.LemonChiffon);
                                Pen selclnoliv = new Pen(Color.LemonChiffon);
                                Rectangle rect88 = new Rectangle(0, CurrentY-10, 870, 25);
                                g.DrawRectangle(selclnoliv, rect88);
                                g.FillRectangle(filloliv, rect88);
                                e.Graphics.DrawString(FieldValue, Verdananormalbld, BlackBrush, 270, CurrentY);

                                FieldValue = _dbtask.znllString(MainGrid.Rows[i].Cells["clncredit"].Value);
                                e.Graphics.DrawString(FieldValue, Verdananormalbld, BlackBrush, 800, CurrentY, Str);
                            
                            
                            }
                            else
                            {
                                e.Graphics.DrawString(FieldValue, Verdananormalbld, BlackBrush, 550, CurrentY, Str);
                           
                                
                            FieldValue = _dbtask.znllString(MainGrid.Rows[i].Cells["clncredit"].Value);
                            e.Graphics.DrawString(FieldValue, Verdananormalbld, BlackBrush, 650, CurrentY, Str);
                            ...
                            e.Graphics.DrawString(FieldValue, Verdananormalbld, BlackBrush, 800, CurrentY, Str);
                                ...
                                FieldValue = _dbtask.znllString(MainGrid.Rows[i].Cells["clnnaration"].Value);

                             if (FieldValue!="")
                            {
                                CurrentY = CurrentY + 15;
                                e.Graphics.DrawString("Narration: " + FieldValue, Verdanasmalls, BlackBrush, 30, CurrentY);
                            ...
                            //FieldValue = String.Format("{0:0.00}", _Dbtask.gridnul(MainGrid.Rows[i].Cells["clndebit"].Value));
                            //e.Graphics.DrawString(FieldValue, Verdananormalbld, BlackBrush, 450, CurrentY);
                            
                            //debt =   _dbtask.znlldoubletoobject(MainGrid.Rows[i].Cells["clndebit"].Value);
                            //if(_dbtask.znllString(MainGrid.Rows[i].Cells["clnvtype"].Value)!="Date wise Balance: ")
                            //{
                            //sumdebt = sumdebt+ debt;
                            //}
                            //if (_dbtask.znllString(MainGrid.Rows[i].Cells["clnvtype"].Value) == "Date wise Balance: ")
                            //{
                            //    sumcrdtDATE = _dbtask.znlldoubletoobject(MainGrid.Rows[i].Cells["clncredit"].Value);
                            //   sumdebtDATE = _dbtask.znlldoubletoobject(MainGrid.Rows[i].Cells["clndebit"].Value);
                            //}
                            ////FieldValue = _dbtask.znllString(MainGrid.Rows[i].Cells["clncredit"].Value);

                            //FieldValue = String.Format("{0:0.00}", _Dbtask.gridnul(MainGrid.Rows[i].Cells["clncredit"].Value));
                            //e.Graphics.DrawString(FieldValue, Verdananormalbld, BlackBrush, 550, CurrentY);
                            //crdt =_dbtask.znlldoubletoobject(MainGrid.Rows[i].Cells["clncredit"].Value);
                            // if(_dbtask.znllString(MainGrid.Rows[i].Cells["clnvtype"].Value)!="Date wise Balance: ")
                            //{
                            //sumcrdt = sumcrdt + crdt;
                            // }
                            //FieldValue = _dbtask.znllString(MainGrid.Rows[i].Cells["clnbalance"].Value);
                            ////FieldValue = FieldValue.Substring(0, 11).ToString();
                            //e.Graphics.DrawString(FieldValue, Verdananormalbld, BlackBrush, 700, CurrentY);
                            
                            // FieldValue = _dbtask.znllString(MainGrid.Rows[i].Cells["clnnarrations"].Value);
                            
                            //if (FieldValue!="")
                            //{
                            // CurrentY = CurrentY + 15;

                            // e.Graphics.DrawString("Note : " + FieldValue, lucidafnt, BlackBrush, 130, CurrentY);
                            ...
                            g.DrawLine(new Pen(Brushes.LightSteelBlue, 1), 0, CurrentY ,870, CurrentY);
                           // g.DrawLine(new Pen(Brushes.Silver, 1), 390, CurrentY , 390, CurrentY + 5);
            ...
            g.DrawLine(new Pen(Brushes.LightSteelBlue, 1), 670, colustart, 670, CurrentY - 10);
            g.DrawLine(new Pen(Brushes.LightSteelBlue, 1), 550, colustart, 550, CurrentY - 10);
            g.DrawLine(new Pen(Brushes.LightSteelBlue, 1), 450, colustart, 450, CurrentY - 10);
           ...
           "Fifty", "Sixty", "Seventy", "Eighty", "Ninety" };
            ...
            public String ConvertAmount(double amount)
            {
                try
                {
                    ...
                    ClsDaybook Frms = new ClsDaybook();
