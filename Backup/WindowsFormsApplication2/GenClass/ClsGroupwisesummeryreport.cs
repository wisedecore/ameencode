...
{
    class ClsGroupwisesummeryreport
    {
        ...
        private Font repofnt = new Font("Castellar", 16, FontStyle.Regular);
       ...
       // private Font InvoiceFont = new Font("Arial", 12, FontStyle.Regular);
        ...
        private Font columnfnt = new Font("Rockwell", 11, FontStyle.Bold);
        ...
        private void SetInvoiceHead(Graphics g)
        {
           ...
           // int ImageHeight = 0;
            ...
            if (System.IO.File.Exists(InvImagehead) && FormType != "ESTIMATE" )
            {
                ...
                ImageHeight = 0;
                //g.DrawImage(oInvImage, 0, 5,750,140);
            }
     ...
     //           g.DrawString(InvSubTitle3, Arialbold, BlackBrush, xInvSubTitle1-20, CurrentY);
            ...
            SolidBrush fillgryc = new SolidBrush(Color.Orange);
            Pen selclnc = new Pen(Color.Orange);
            Rectangle rectc = new Rectangle(0, 50, 850, 60);
            g.DrawRectangle(selclnc, rectc);
            g.FillRectangle(fillgryc, rectc);
            g.DrawString(InvTitle, repofnt, BlackBrush, 150, 70);
            g.DrawString("VAT : "+InvSubTitle3, repofnt, BlackBrush, 150, 90);


            SolidBrush fillgryde = new SolidBrush(Color.Orange);
            Pen selclnde = new Pen(Color.Orange);
            Rectangle rectde = new Rectangle(0, 140, 850, 30);
            g.DrawRectangle(selclnde, rectde);
            g.FillRectangle(fillgryde, rectde);
            g.DrawString(FormType, repofnt, BlackBrush, 100, 140);




            SolidBrush fillgryh = new SolidBrush(Color.Orange);
            Pen selclnh = new Pen(Color.Orange);
            Rectangle recth = new Rectangle(5, 240, rightMargin + 20, 30);
            g.DrawRectangle(selclnh, recth);
            g.FillRectangle(fillgryh, recth);
                ...
                //g.DrawString(InvSubTitle5, Arialbold, BlackBrush,250, 100);
       ...
       //     CurrentY = CurrentY + InvSubTitleHeight + 8;
        ...
        private void SetOrderData(Graphics g)
        {
            ...
            string repname = "";
            repname = FormType;
            g.DrawString(CName, adrssfnt, BlackBrush, 20, CurrentY);
            //g.DrawString(timestr, VerdanaBold, BlackBrush, 750, CurrentY,Str);
            CurrentY = CurrentY + 15;
            g.DrawString("Contact : "+mob, VerdanaBold, BlackBrush, 20, CurrentY);


            g.DrawString("Tax Number :-"+CTin, VerdanaBold, BlackBrush, 750, CurrentY, Str);
            CurrentY = CurrentY + 15;
        ...
        {// Set Invoice Table:
          ...
          //\ g.DrawString("SlNo", Verdananormal, BlackBrush, xSlno, CurrentY);
            int xProductName;
            int xRate;
            SolidBrush fillgry = new SolidBrush(Color.PapayaWhip);
            Pen selcln = new Pen(Color.PapayaWhip);
            Rectangle rect8 = new Rectangle(5, CurrentY, rightMargin+20, 30);
            g.DrawRectangle(selcln, rect8);
            g.FillRectangle(fillgry, rect8);
            
                g.DrawString("Date ", VerdanaBold2, BlackBrush, 30, CurrentY);
                g.DrawString("Voucher ", VerdanaBold2, BlackBrush, 100, CurrentY);
                g.DrawString("Bill Amount ", VerdanaBold2, BlackBrush, 200, CurrentY);

                if (FormType == "Sales Billwise Settlement Details" || FormType == "Purchase Return Billwise Settlement Details")
                {
                g.DrawString("Received ", VerdanaBold2, BlackBrush, 400, CurrentY);
                }
                else
                {
                 g.DrawString("Paided ", VerdanaBold2, BlackBrush, 400, CurrentY);
                }
            
                 g.DrawString("Balance ", VerdanaBold2, BlackBrush, 650, CurrentY);
          ...
          //  RowValidation(MainGrid);
           ...
           //CurrentY = CurrentY + InvoiceFontHeight + 2;
            ...
            //if (= "Purchase Return Billwise Settlement Details")
            //{
                    ...
                    for (int i = TRowcounting; i < MainGrid.Rows.Count - 1; i++)
                    {
                        ...
                        if (MainGrid.Rows[i].Cells["clnactualbill"].Value != null &&_dbtask.znllString( MainGrid.Rows[i].Cells["clnactualbill"].Value) !="")
                        {
                            if (i>0)
                            {
                        if (TempRowcounting < RecordsPerPage)
                        {
                            Rectangle rect9 = new Rectangle(620, CurrentY, 150, 20);
                            g.DrawRectangle(rowclr, rect9);
                            g.FillRectangle(grycln, rect9);
                            ...
                            e.Graphics.DrawString(FieldValue, columnfnt, BlackBrush, 20, CurrentY);

                            FieldValue = _dbtask.znllString(MainGrid.Rows[i].Cells["clnvno"].Value);
                            e.Graphics.DrawString(FieldValue, columnfnt, BlackBrush, 130, CurrentY);

                            FieldValue = _dbtask.znllString(MainGrid.Rows[i].Cells["clnactualbill"].Value);
                            e.Graphics.DrawString(FieldValue, columnfnt, BlackBrush, 200, CurrentY);

                            FieldValue = _dbtask.znllString(MainGrid.Rows[i].Cells["clnagainamt"].Value);
                            e.Graphics.DrawString(FieldValue, columnfnt, BlackBrush, 400, CurrentY);
                            //debt = _dbtask.znlldoubletoobject(MainGrid.Rows[i].Cells["clnBALANCE"].Value);
                            
                            //sumdebt = sumdebt+ debt;

                            FieldValue = _dbtask.znllString(MainGrid.Rows[i].Cells["clnBALANCE"].Value);
                            e.Graphics.DrawString(FieldValue, columnfnt, BlackBrush, 750, CurrentY, Str);
                            //crdt =_dbtask.znlldoubletoobject(MainGrid.Rows[i].Cells["clncredit"].Value);
                            //sumcrdt = sumcrdt + crdt;

                            //FieldValue = _dbtask.znllString(MainGrid.Rows[i].Cells["clnbalance"].Value);
                            ////FieldValue = FieldValue.Substring(0, 11).ToString();
                            //e.Graphics.DrawString(FieldValue, Verdananormal, BlackBrush, 750, CurrentY,Str);
                            ...
                            //e.Graphics.DrawString(FieldValue, Verdananormal, BlackBrush, 130, CurrentY);
           ...
           "Fifty", "Sixty", "Seventy", "Eighty", "Ninety" };
            ...
            public String ConvertAmount(double amount)
            {
                try
                {
                    ...
                    ClsGroupwisesummeryreport Frms = new ClsGroupwisesummeryreport();
      ...
      //      CurrentY = CurrentY + 16;
        ...
        public void PrintTotal(Graphics g)
        {
            ...
            SolidBrush fillgry = new SolidBrush(Color.Gainsboro);
            Pen selcln = new Pen(Color.Black);
            Rectangle rect8 = new Rectangle(5, CurrentY, 780, 20);
            g.DrawRectangle(selcln, rect8);
            g.FillRectangle(fillgry, rect8);
            //g.DrawLine(new Pen(Brushes.Black, 1), leftMargin, CurrentY, rightMargin, CurrentY);
            double blceofparty=0;
            blceofparty= sumdebt - sumcrdt;
            g.DrawString("Balance : "  + _dbtask.znllString( blceofparty), Arialbold, BlackBrush,770, CurrentY,Str);
            g.DrawString("Cr : "+_dbtask.znllString(sumdebt) + "        " +"Db :"+ _dbtask.znllString(sumcrdt), Arialbold, BlackBrush, 550, CurrentY, Str);
