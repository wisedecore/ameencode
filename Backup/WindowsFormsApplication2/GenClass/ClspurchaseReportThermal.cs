...
{
    class ClspurchaseReportThermal
    {
        int hx = 5;
      ...
      int xRate; public string StrNaration = "";
        ...
        double SUmNetAmt = 0; double SUmqty = 0;
        double SumSrate = 0; double Sumgrss = 0;
        int XGrossAmt;
        ...
        private void SetOrderData(Graphics g)
        {
            ...
            {
                CurrentY = 100;
                g.DrawString("PURCHASE REPORT", VerdanaBoldHeadingMain, BlackBrush, 60, CurrentY);
           ...
           CurrentY = CurrentY + 16;
            ...
            if (FormType == "Purchase A4")
            {
                ...
                CurrentY = 80;
                g.DrawString("PURCHASE REPORT", VerdanaBoldHeadingMain, BlackBrush, 300, CurrentY);
           ...
           // CurrentY = CurrentY + 20;
            ...
            if (FormType == "Purchase A4")
            {
                ...
                g.DrawString("Date", VerdanaBold, BlackBrush, 0, CurrentY);
                g.DrawString("Vno", VerdanaBold, BlackBrush, 120, CurrentY);
                g.DrawString("Party", VerdanaBold, BlackBrush, 170, CurrentY);
                g.DrawString("Qty", VerdanaBold, BlackBrush, 450, CurrentY, Str);
                g.DrawString("Tax", VerdanaBold, BlackBrush,550, CurrentY, Str);

                g.DrawString("Amount", VerdanaBold, BlackBrush, 600, CurrentY); 
                g.DrawString("NetAmount", VerdanaBold, BlackBrush, 750, CurrentY, Str);
                ...
                for (int i = TRowcounting; i < MainGrid.Rows.Count - 1; i++)
                {
                    if (TempRowcounting < RecordsPerPage)
                    {
                        if (_Dbtask.znllString(MainGrid.Rows[i].Cells["vno"].Value) != "")
                        {
                            ...
                            FieldValue = _Dbtask.znllString(MainGrid.Rows[i].Cells["vno"].Value);
                            e.Graphics.DrawString(FieldValue, VerdanaBold, BlackBrush,120, CurrentY);


                            FieldValue = _Dbtask.znllString(MainGrid.Rows[i].Cells["party"].Value);
                            e.Graphics.DrawString(FieldValue, Declarationfont, BlackBrush, 170, CurrentY);



                            FieldValue = String.Format("{0:0.}", _Dbtask.gridnul(MainGrid.Rows[i].Cells["qty"].Value));
                            e.Graphics.DrawString(FieldValue, VerdanaArabic, BlackBrush,450, CurrentY);
                            SUmqty = SUmqty + CommonClass._Dbtask.znlldoubletoobject(FieldValue);
           ...
           "Fifty", "Sixty", "Seventy", "Eighty", "Ninety" };
            ...
            public String ConvertAmount(double amount)
            {
                try
                {
                    ...
                    ClspurchaseReportThermal Frms = new ClspurchaseReportThermal();
