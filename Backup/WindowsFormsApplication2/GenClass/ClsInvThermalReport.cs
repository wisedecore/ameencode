    ...
    class ClsInvThermalReport
    {
      ...
      public  string FieldValue;
        ...
        private void ReadInvoiceHead()
            ...
            else if (FormType == "Account Report") {
                InvSubTitle5 = "      " + FormType;
            }
            else
            {
                ...
                
        {
            ...
            if (FormType == "Account Report")
            {
                InvSubTitle5 = "Account Report";//, VerdanaBold, BlackBrush, 550, CurrentY);//142
            ...
            if (InvTitle != "" && FormType != "Purchase Order" )/*For Company Heading*/
            {
                ...
                if (FormType != "Salesdaybookcount")
                {
                    hx = (300 - (int)g.MeasureString(InvTitle, arbHead).Width) / 2;
                    CurrentY = CurrentY + 5;
            ...
            {
               hx= (300 - (int)g.MeasureString(NameInHome, arbHead2).Width) / 2;
            ...
            if (InvSubTitle1 != "" && FormType != "     SALES ORDER" && FormType != "Purchase Order" && FormType != "SALES QUATATION" && FormType != "Salesdaybookcount")/*For Address*/
            {
              ...
              hx = (300 - (int)g.MeasureString(InvSubTitle1, arbHead).Width) / 2;
                ...
                if (InvSubTitle2 != "" && FormType != "Salesdaybookcount" && FormType != "Report" && FormType != "     SALES ORDER" && FormType != "Purchase Order" && FormType != "SALES QUATATION")/*For Phone*/
            {
               ...
               hx = (300 - (int)g.MeasureString(InvSubTitle2, arbHead2).Width) / 2;
                ...
                if (InvSubTitle3 != "" && FormType != "Report" && FormType != "Salesdaybookcount" && FormType != "     SALES ORDER" && FormType != "Purchase Order" && FormType != "SALES QUATATION")/*VAt Number*/
            {
               ...
               g.DrawString(InvSubTitle3, arbHead2, BlackBrush, hx - 20, CurrentY);
                CurrentY = CurrentY + 7;
            }
            ...
            if (FormType == "Account Report")
            {
                ...
                g.DrawString("Account Report", VerdanaBold, BlackBrush, 70, CurrentY);
            ...
            //    g.DrawString(FieldValue, VerdanaBold, BlackBrush, 3, CurrentY);
             ...
             //   CurrentY = CurrentY + 16;
              ...
              // employee = _dbtask.znllString(_Accountledger.GetspecifField("lname", employeid));
               Pnamee = _dbtask.znllString(_Accountledger.GetspecifField("lname", TEmpparty));
               ...
               if (CommonClass._Menusettings.GetMnustatus("200").ToString() == "1")
               {
                ...
                g.DrawString("Party : "+Pnamee, VerdanaBold, BlackBrush, 3, CurrentY);
                FieldValue = CName;
                CurrentY = CurrentY + 5;
        ...
        {// Set Invoice Table:
            ...
            {
                //xSlno = leftMargin;
               g.DrawString("VNO", VerdanaBold, BlackBrush, CurrentX, CurrentY);
            ...
            {
  g.DrawLine(new Pen(Brushes.Black, 1), leftMargin, CurrentY, 270, CurrentY);
CurrentY = CurrentY + 8;
                //xSlno = leftMargin;
                g.DrawString("VNO", VerdanaBold, BlackBrush, CurrentX, CurrentY);
                CurrentX = CurrentX + 30;
                g.DrawString("Amount", VerdanaBold, BlackBrush, CurrentX+60, CurrentY);
                CurrentX = CurrentX + 60;
                g.DrawString("Balance", VerdanaBold, BlackBrush, CurrentX+100, CurrentY);
                CurrentY = CurrentY + 15;
                g.DrawLine(new Pen(Brushes.Black, 1), leftMargin, CurrentY, 270, CurrentY);

                CurrentY = CurrentY + 8;
                //xProductName = xSlno + (int)g.MeasureString("VNO", VerdanaBold).Width;
                ...
                if (FormType != "Salesdaybookcount")
                {
                    ...
                    for (int i = TRowcounting; i < MainGrid.Rows.Count - 1; i++)
                    {
                        ...
                        if (MainGrid.Rows[i].Cells["clndate"].Value != null && MainGrid.Rows[i].Cells["clndate"].Value != "Ledger")
                        {
                            ...
                            if (FieldValue != "")
                            {
                                g.DrawString(FieldValue, Verdananormal, BlackBrush, 0, CurrentY);
                            }
                           ...
                           CurrentY = CurrentY + 15;
                            //g.DrawLine(new Pen(Brushes.Black, 1), 0, CurrentY, 750, CurrentY);
                            ...
                            CurrentY = CurrentY + 7;
                            g.DrawString("-------------------------------------------------------", Verdananormal, BlackBrush, 0, CurrentY);
                        ...
                        //{
                            //FieldValue = _dbtask.znllString(MainGrid.Rows[i].Cells["clnvno"].Value);
                            //e.Graphics.DrawString(FieldValue, VerdanaBold, BlackBrush, 13, CurrentY + 10);
                            ...
                            //else
                            //{
                            //FieldValue =_dbtask.znllString(MainGrid.Rows[i].Cells["clnamount"].Value);
                            //e.Graphics.DrawString(FieldValue, VerdanaBold, BlackBrush, xSlno, CurrentY);
                            ...
                            //    dbandcr = (debit + credit);
                            //    FieldValue = _Dbtask.SetintoDecimalpoint(dbandcr).ToString();
                            //    e.Graphics.DrawString(FieldValue, VerdanaBold, BlackBrush, CurrentX + 20, CurrentY + 10);

                            //    FieldValue = _Dbtask.znllString(MainGrid.Rows[i].Cells["clnbalance"].Value);
                            //    g.DrawString(FieldValue, Verdananormal, BlackBrush, CurrentX + 150, CurrentY + 10);
                            //    if (FieldValue != "")
                            //    {
                            //        g.DrawString(FieldValue, Verdananormal, BlackBrush, CurrentX + 150, CurrentY + 10);
                            //        SUmNetAmt = _Dbtask.znlldoubletoobject(FieldValue);
            ...
            for (int i = TRowcounting; i < MainGrid.Rows.Count-1 ; i++)
            {
                ...
                //if (_Dbtask.znllString(MainGrid.Rows[i].Cells["clndate"].Tag) == "1")
                //{

                    string Itemid =_dbtask.znllString( MainGrid.Rows[i].Cells["clndate"].Tag);
           ...
           "Fifty", "Sixty", "Seventy", "Eighty", "Ninety" };
            ...
            public String ConvertAmount(double amount)
            {
                try
                {
                    ...
                    ClsInvThermalReport Frms = new ClsInvThermalReport();
          ...
          //g.DrawString(CommonClass._Dbtask.SetintoDecimalpoint(Convert.ToDouble(TDiscount)), VerdanaBold, BlackBrush, 105, CurrentY);
            ...
            else
            {
                ...
                if (FormType != "Salesdaybookcount")
                    ...
                    CurrentY = CurrentY + 12;//28//25
                    
                    g.DrawString("     TOTAL  AMOUNT:"+SUmNetAmt.ToString(), CalibiRiItalic3, BlackBrush, 30, CurrentY);
        ...
        private void SetInvoiceTotal(Graphics g)
          ...
          //  g.DrawString(FieldValue, CalibiRiItalic, BlackBrush, 2, CurrentY-15);
           ...
           // g.DrawString("jjj", VerdanaBold, BlackBrush, 2, CurrentY, Str);
            ...
            if (InvSubTitle5 != "       Report INVOICE" && FormType != "Salesdaybookcount" && FormType != "Account Report")
            {
