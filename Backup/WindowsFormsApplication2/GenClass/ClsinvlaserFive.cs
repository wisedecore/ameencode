    ...
    class ClsinvlaserFive
    {
        ...
        public void PrintTotal(Graphics g)
        {
            ...
            if (InvSubTitle5 != "DELIVERY NOTE INVOICE")
            {
                ...
                g.DrawString("Total", VerdanaBold, BlackBrush, 150, CurrentY);
                g.DrawString(TotalQty, Verdananormal, BlackBrush, 410, CurrentY);//770/*For Qty*///350
                g.DrawString(_Dbtask.SetintoDecimalpoint(TotalTaxable), Verdananormal, BlackBrush, 628, CurrentY, Str);/*For Qty*///350
                ...
                g.DrawString(_Dbtask.SetintoDecimalpoint(TotalTaxAmount), Verdananormal, BlackBrush, 680, CurrentY, Str);/*For Tax*/
                g.DrawString(_Dbtask.SetintoDecimalpoint(TotalNetamount), Verdananormal, BlackBrush, 755, CurrentY, Str);/*For Amount*/

                CurrentY = CurrentY;
                CurrentY = CurrentY + 20;
                CurrentY = CurrentY + (800 - CurrentY);//800

                g.DrawRectangle(selpen, 10, CurrentY, 150, 90);
                g.DrawRectangle(selpen, 300, CurrentY, 482, 150);
                ...
                g.DrawString("Discount   " + _Gen.FuReturnArabic("Discount") + "", Verdananormal, BlackBrush, 305, CurrentY);
                if (Convert.ToDouble(TDiscount) > 0)
                {
                    if (isarbicdotprint == true)
                    {
                ...
                g.DrawLine(new Pen(Brushes.Black, 1), 300, 835, 781, CurrentY);

                CurrentY = CurrentY + 10;//845
                g.DrawString("Tot.Amount(exclusive vat) " + _Gen.FuReturnArabic("Excl Vat") + "", Verdananormal, BlackBrush, 300, CurrentY);
                if (isarbicdotprint == true)
                {
                ...
                g.DrawLine(new Pen(Brushes.Black, 1), 300, CurrentY, 781, CurrentY);

                CurrentY = CurrentY + 9;//885

                g.DrawString("Total vat 15%   " + _Gen.FuReturnArabic("Total Vat") + "", Verdananormal, BlackBrush, 300, CurrentY);
                if (isarbicdotprint == true)
                {
                    xTotalValue = 750 - (int)g.MeasureString(BillAmount, Arialbold).Width;
                    //g.DrawString(_Dbtask.SetintoDecimalpoint(TotalTaxable).ToString(), Arialboldbottm, BlackBrush, 750, CurrentY, Str);
                ...
                g.DrawString("Total Amount   " + _Gen.FuReturnArabic("Amount Total") + "", Verdananormal, BlackBrush, 305, CurrentY);

                CurrentY = CurrentY + 33;//960
                g.DrawRectangle(selpen2, 160, CurrentY, 624, 50);
        ...
        public void setqrtwo(Graphics g)
        {
            ...
            g.DrawImage(newbit, 180, CurrentY);
            ...
            if (FormType != "DELIVERY NOTE")
            {
                ...
                if (isarbicdotprint == true)
                {
                    ...
                    if (amtwrd == 1)
                    {
                        ...
                        g.DrawString(" " + AmountinWords + " only", AmountInwordsfont3, BlackBrush, 180, CurrentY);
                        //arabic
