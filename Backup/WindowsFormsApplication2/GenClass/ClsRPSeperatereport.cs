...
namespace WindowsFormsApplication2
{
    class ClsRPSeperatereport
    {
        ...
        public static bool rptax = false;
       ...
       // private Font InvoiceFont = new Font("Arial", 12, FontStyle.Regular);
        ...
        private Font Verdantax= new Font("Verdana", 7, FontStyle.Bold);

        private Font italic = new Font("Lucida Bright", 8, FontStyle.Italic);
     ...
     //           g.DrawString(InvSubTitle3, Arialbold, BlackBrush, xInvSubTitle1-20, CurrentY);
            ...
            if (FormType == "Receipt Report only" || FormType == "Payment Report only")
            {
                if( head == false)
                {
            ...
            if (FormType == "Receipt Report only" || FormType == "Payment Report only")
            {
                ...
                {
                    g.DrawString("Payment Report ", gothicfnt, BlackBrush, 300, CurrentY);
                    CurrentY = CurrentY + 23;
                }
       ...
       //     CurrentY = CurrentY + InvSubTitleHeight + 8;
        ...
        {// Set Invoice Table:
          ...
          //\ g.DrawString("SlNo", Verdananormal, BlackBrush, xSlno, CurrentY);
            int xProductName;
            int xRate;
            SolidBrush fillgry = new SolidBrush(Color.LavenderBlush);
            Pen selcln = new Pen(Color.LavenderBlush);
            Rectangle rect8 = new Rectangle(0, CurrentY, 870, 20);
            g.DrawRectangle(selcln, rect8);
            g.FillRectangle(fillgry, rect8);
            if (FormType == "Payment Report only" || FormType == "Receipt Report only")

            {
                ...
                g.DrawString("Party", VerdanaBold2, BlackBrush, 300, CurrentY);
                CurrentX = CurrentX + 80;
                ...
                if(rptax==true)
                {
                    ...
                    g.DrawString("Tax", VerdanaBold2, BlackBrush, 630, CurrentY);
                    CurrentX = CurrentX + 10;
                }


               
                g.DrawString("Amount", VerdanaBold2, BlackBrush, 750, CurrentY);
                CurrentX = CurrentX + 10;
          ...
          //  RowValidation(MainGrid);
           ...
           //CurrentY = CurrentY + InvoiceFontHeight + 2;
            ...
            Pen rowclr = new Pen(Color.LavenderBlush);
            ...
            if (FormType == "Payment Report only" || FormType == "Receipt Report only")
            {
                    ...
                    for (int i = TRowcounting; i <= MainGrid.Rows.Count - 1; i++)
                    {
                        ...
                        if ( FormType == "Receipt Report only")
                        {
                            ...
                            if (_dbtask.znllString(MainGrid.Rows[i].Cells["clnramount"].Value) != "" && _dbtask.znllString(MainGrid.Rows[i].Cells["clnramount"].Value) != "")
                            {
                                ...
                                if (TempRowcounting < RecordsPerPage)
                                {
                                    ...
                                    FieldValue = _dbtask.znllString(MainGrid.Rows[i].Cells["clnrdate"].Value);
                                    ...
                                    if (rptax == true)
                                    {
                                        ...
                                        e.Graphics.DrawString(FieldValue, Verdantax, BlackBrush, 300, CurrentY);
                                    }
                                    else
                                    {
                                        FieldValue = _dbtask.znllString(MainGrid.Rows[i].Cells["clnreceipt"].Value);
                                    ...
                                    if(rptax==true)
                                    {
                                    FieldValue = String.Format("{0:0.00}", _Dbtask.gridnul(MainGrid.Rows[i].Cells["clnrtaxamt"].Value));
                                    ...
                                    FieldValue = _dbtask.znllString(MainGrid.Rows[i].Cells["clnNOTE"].Value);
                                    e.Graphics.DrawString("Note : " + FieldValue, italic, BlackBrush, 30, CurrentY);


                                    FieldValue = _dbtask.znllString(MainGrid.Rows[i].Cells["clnREF"].Value);
                                    e.Graphics.DrawString( FieldValue +": RefNo", italic, BlackBrush,800, CurrentY,Str);

                                    //g.DrawLine(new Pen(Brushes.Silver, 1), 390, CurrentY - 5, 390, CurrentY + 17);
                                    CurrentY = CurrentY + 15;

                                    g.DrawLine(new Pen(Brushes.LavenderBlush, 1), 0, CurrentY + 5, 870, CurrentY + 5);
                        ...
                        if (FormType == "Payment Report only")
                        {
                            ...
                            if (_dbtask.znllString(MainGrid.Rows[i].Cells["clnpamount"].Value) != "" && _dbtask.znllString(MainGrid.Rows[i].Cells["clnpamount"].Value) != "")
                            {
                                ...
                                if (TempRowcounting < RecordsPerPage)
                                {
                                    ...
                                    if (rptax == true)
                                    {
                                        FieldValue = String.Format("{0:0.00}", _Dbtask.gridnul(MainGrid.Rows[i].Cells["clnptaxamt"].Value));
                                        e.Graphics.DrawString(FieldValue, Verdananormalbld, BlackBrush, 670, CurrentY, Str);
           ...
           "Fifty", "Sixty", "Seventy", "Eighty", "Ninety" };
            ...
            public String ConvertAmount(double amount)
            {
                try
                {
                    ...
                    ClsRPSeperatereport Frms = new ClsRPSeperatereport();
