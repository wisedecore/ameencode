...
namespace WindowsFormsApplication2
{
    class Clsinvlaser1
    {
        ...
        private Font Verdananormal1 = new Font("Verdana", 12, FontStyle.Bold);
        ...
        private void SetInvoiceHead(Graphics g)
        {
            ...
            {
                //g.DrawString("TRN:", Arialnormal, BlackBrush, 50, CurrentY / 2);/*For Tin*/
            ...
            if (InvSubTitle5 != "")/*For InVoiceName*/
            {
                ...
                g.DrawString(BillDate.ToString("dd/MM/yyyy"), Arialbold, BlackBrush, 675, 192);
                g.DrawString(Billno, Arialbold, BlackBrush, 420, 155);
                CurrentY = CurrentY + 5;
        ...
        private void SetOrderData(Graphics g)
        {
            ...
            g.DrawString(CTin, Arialbold, BlackBrush, 285, 192);
            if (SProject == true)
            {
            ...
            CName = CName.PadLeft(50 - CName.Length, ' ');
            g.DrawString(FieldValue, Arialbold, BlackBrush, 525, 225);
        ...
        {// Set Invoice Table:
            ...
            // g.DrawString("Amount", Verdananormal, BlackBrush, 737, CurrentY);
            ...
            for (int i = TRowcounting; i < MainGrid.Rows.Count - 1; i++)
            {
                ...
                if (_Dbtask.znllString(MainGrid.Rows[i].Tag) == "1")
                {
                    if (TempRowcounting < RecordsPerPage)
                    {
                        ...
                        FieldValue = MainGrid.Rows[i].Cells["clnbatch"].Value.ToString();
                        g.DrawString(FieldValue, Verdananormal, BlackBrush, xSlno + 35, CurrentY);
                        ...
                        g.DrawString(FieldValue, Verdananormal1, BlackBrush, 550 - (int)g.MeasureString(FieldValue, Verdananormal1).Width, CurrentY);
                        ...
                        g.DrawString(FieldValue, Verdananormal, BlackBrush, 695, CurrentY, Str);
           ...
           "Fifty", "Sixty", "Seventy", "Eighty", "Ninety" };
            ...
            public String ConvertAmount(double amount)
            {
                try
                {
                    ...
                    Clsinvlaser1 Frms = new Clsinvlaser1();
        ...
        {// Set Invoice Total:
            ...
            CurrentY = CurrentY - 66;
            // g.DrawString(TotalValue, Arialbold, BlackBrush, 600, CurrentY);

           
           // g.DrawString(TotalValue, Arialbold, BlackBrush, 700, CurrentY);

            CurrentY = CurrentY + (int)g.MeasureString(TotalValue, Arialbold).Height * 2;
            TotalValue = TotalTaxable.ToString("0.00");
            g.DrawString(TotalValue, Arialbold, BlackBrush, 750, CurrentY);

            CurrentY = CurrentY + (int)g.MeasureString(TotalValue, Arialbold).Height * 2;
            TotalValue = TotalTaxAmount.ToString("0.00");
            g.DrawString(TotalValue, Arialbold, BlackBrush, 750, CurrentY);

            CurrentY = CurrentY + (int)g.MeasureString(TotalValue, Arialbold).Height * 2;
            TotalValue = _Dbtask.znullDouble(BillAmount).ToString("0.00");
            g.DrawString(TotalValue, Arialbold, BlackBrush, 750, CurrentY);
