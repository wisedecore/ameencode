...
public class MD5HashGenerator
{
    ...
    public static String GenerateKeySERVICE(Object sourceObject)
    {
        String hashString = "";
        ...
        else
        {
            ...
            {
                //Now we begin to do the real work.
                hashString = ComputeHashSERVCE(ObjectToByteArray(sourceObject));
                return hashString;
            }
            catch (AmbiguousMatchException ame)
            {
	...
	private static string ComputeHash(byte[] objectAsBytes)
	{
		...
		try
		{
			...
			{
				sb.Append(result[i].ToString("X1"));
			}
    ...
    private static string ComputeHashSERVCE(byte[] objectAsBytes)
    {
        MD5 md5 = new MD5CryptoServiceProvider();
        try
        {
