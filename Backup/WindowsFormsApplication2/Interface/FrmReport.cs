...
namespace WindowsFormsApplication2
{
   ...
   //  private System.ComponentModel.Container components;
        ...
        public void ReportShowInGrid()
            ...
            try
            {
                ...
                if (ReportType == "OutstandingReportsummery")
                {
                    //int count;
                    double TTbalance = 0;
                    Ds = _Dbtask.ExecuteQuery(QueryTemp);
                    for (int i = 0; i < Ds.Tables[0].Rows.Count; i++)
                    {
                        string Ledid = Ds.Tables[0].Rows[i]["lid"].ToString();
                        double Tempbalace = Convert.ToDouble(_Dbtask.ExecuteScalar("select isnull (sum(dbamt-cramt),0) as amt from tblgeneralledger where ledcode='" + Ledid + "' and vdate between '" + DTFrom.ToString("MM/dd/yyyy 00:00:00") + "' and '" + DTTo.ToString("MM/dd/yyyy 23:59:59") + "' "));

                        if (Tempbalace != 0)
                        {
                            int count2;
                            string PartyId = Ledid;
                            string PartyName = _Dbtask.ExecuteScalar("select lname from tblaccountledger where lid='" + PartyId + "'");
                ...
                if (ReportType == "AccountReport")
                    ...
                    for (int i = 0; i < Ds.Tables[0].Rows.Count; i++)
                    {
                        ...
                        string Opbalance = _Dbtask.ExecuteScalar("select isnull(sum(dbamt-cramt),0) from tblgeneralledger where ledcode='" + Ledid + "' and vDate <'" + DTFrom.ToString("MM/dd/yyyy 00:00:00") + "'");
                      ...
                      // Query="select * from tblgeneralledger  where ledcode in ("+Ledid+")";
                        ...
                        Ds1 = _Dbtask.ExecuteQuery(Query + " and vDate between '" + DTFrom.ToString("MM/dd/yyyy 00:00:00") + "' and '" + DTTo.ToString("MM/dd/yyyy 23:59:59") + "' order by vdate asc");
                        for (int ii = 0; ii < Ds1.Tables[0].Rows.Count; ii++)
                        {
                           Count= GridMain.Rows.Add(1);
                            Temprow = Temprow + 1;
                            ...
                            if (Vtype == "SI" || Vtype == "PR" || Vtype == "SO" || Vtype == "SV")
                            {
                                ...
                                {
                                    ClickCode = _Dbtask.ExecuteScalar("select issuecode from tblbusinessissue where vno='" + Vno.Replace(CommonClass._Paramlist.GetParamvalue("Sprefix"), "") + "' and ledcodecr='" + RefNo + "' and issuetype='" + Vtype + "'");
                            ...
                            if (Vtype == "SI" )
                            {
                                ...
                                DUE = _Dbtask.znllString( _Dbtask.ExecuteScalar("SELECT due from tblbusinessissue where issuecode='" + Vno + "'"));
                            }
                        ...
                        Ds1 = _Dbtask.ExecuteQuery("select * from Tblbusinessissue where LedcodeCr in (" + Ledid + ") and IssueDate between '" + DTFrom.ToString("MM/dd/yyyy 00:00:00") + "' and '" + DTTo.ToString("MM/dd/yyyy 23:59:59") + "' and IssueType='SO' order by IssueDate asc");
                        for (int ii = 0; ii < Ds1.Tables[0].Rows.Count; ii++)
                        {
                            Count = GridMain.Rows.Add(1);
                            Temprow = Temprow + 1;
                            GridMain.Rows[Count].Cells["clndate"].Value = Ds1.Tables[0].Rows[ii]["IssueDate"];
                            string Purticulars = _Dbtask.ExecuteScalar("select lname from tblaccountledger where lid='" + Ds1.Tables[0].Rows[i]["LedcodeCr"] + "'");
                            //string notess = "";
                        ...
                        Ds1 = _Dbtask.ExecuteQuery("select * from TblTransactionReceipt where LedcodeDr in (" + Ledid + ") and RecptDate between '" + DTFrom.ToString("MM/dd/yyyy 00:00:00") + "' and '" + DTTo.ToString("MM/dd/yyyy 23:59:59") + "' and RecptType='PO' order by RecptDate asc");
                        for (int ii = 0; ii < Ds1.Tables[0].Rows.Count; ii++)
                        {
                            Count = GridMain.Rows.Add(1);
                            Temprow = Temprow + 1;
                            GridMain.Rows[Count].Cells["clndate"].Value = Ds1.Tables[0].Rows[ii]["RecptDate"];
                            string Purticulars = _Dbtask.ExecuteScalar("select lname from tblaccountledger where lid='" + Ds1.Tables[0].Rows[i]["LedcodeDr"] + "'");
                ...
                if (ReportType == "OutstandingReport")
                {
                    ...
                    for (int i = 0; i < Ds.Tables[0].Rows.Count; i++)
                    {
                        ...
                        if (CommonClass._SelectReport.Reportselect(Tempbalace)==true)
                        {
                            ...
                            double BalanceTemp = 0;
                            double Days;
                            Ds1 = _Dbtask.ExecuteQuery("select * from tblgeneralledger where   ledcode='" + Ledid + "'" + " and vDate between '" + DTFrom.ToString("MM/dd/yyyy 00:00:00") + "' and '" + DTTo.ToString("MM/dd/yyyy 23:59:59") + "' order by vdate asc");
                            for (int ii = 0; ii < Ds1.Tables[0].Rows.Count; ii++)
                            {
        ...
        public void DrillDown()
            ...
            if (LblHeading.Text == "Sales" || LblHeading.Text == "Sales Order" || LblHeading.Text == "Sales Quatation" || LblHeading.Text=="Services")
            {
                if (GridMain.SelectedCells.Count > 0)
                {
                    SelectedRow = GridMain.SelectedCells[0].RowIndex;
